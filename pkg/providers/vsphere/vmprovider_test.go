...
func vmSnapshotTests() {
	...
	BeforeEach(func() {
		...
		vm = builder.DummyBasicVirtualMachine("test-snapshot", nsInfo.Namespace)
		vm.Status.UniqueID = vcVM.Reference().Value
		vmSnapshot = builder.DummyVirtualMachineSnapshot("test-snapshot", nsInfo.Namespace, vcVM.Name())
	...
	Describe("DeleteSnapshot", func() {
		JustBeforeEach(func() {
			err = vmProvider.DeleteSnapshot(ctx, vmSnapshot, vm, true, nil)
		})

		It("should delete the snapshot", func() {
		...
		Context("VM is not found", func() {
			...
			It("should return an error", func() {
				Expect(err).To(HaveOccurred())
				Expect(errors.Is(err, vcenter.ErrVMNotFound)).To(BeTrue())
			})
		})
		...
		Context("snapshot not found", func() {
			...
			It("returns error", func() {
				Expect(err).To(HaveOccurred())
				Expect(errors.Is(err, virtualmachine.ErrVMSnapshotNotFound)).To(BeTrue())
			})
		})
	})
}
