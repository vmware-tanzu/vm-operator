...
- **E2E** (mandatory per `e2e-sync-with-changes.md` — cluster-observable behavior), status per scenario:
  ...
  3. **Done** (`tasks.md` T029): a 3-VM spec in `vmevictionpolicy.go` drives this against a real host. VM1 (unpinned) is relocated off the host entering maintenance by DRS and stays `Synced`. VM2 and VM3 mimic a real-world VM that DRS cannot evacuate (e.g. one with a PCI-passthrough/GPU device) by pinning each to VM1's host via a real, directly-created `vm_host_affinity` `ComputePolicy`/`InfraPolicy` (`pinVMToHost`), gated by this `Spec`'s existing `consts.VMEvictionCapabilityName` check like every other `It` in the file. VM2/VM3 are then power-cycled off/on while the host is in maintenance, asserting `VirtualMachinePowerStateSynced=False` with reason `InfraInMaintenance` while `spec.powerState=PoweredOn` and `status.powerState=PoweredOff`; both revert to `Synced` once the host exits maintenance mode, VM2 confirmed back on the same host (strict pin), VM3 (which also explicitly references the Optional `BestEffortRestartPolicy`) with no host assertion. Skips on a single-host cluster. The `VMEviction`-off path isn't separately exercised — the whole spec file is skipped via capability gating when the feature is off, and that baseline is already covered elsewhere.
