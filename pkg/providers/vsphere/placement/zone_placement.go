...
func getDatastoreProperties(
	...
	if err := pc.Retrieve(
		...
		dsRefs,
		[]string{
			// TODO(akutz) There is a bug in GoVmomi that is preventing getting
			//             the property info.supportedVDiskFormats via the
			//             explicit path. Doug Mac is working on a fix, but
			//             until then, just get all of info.
