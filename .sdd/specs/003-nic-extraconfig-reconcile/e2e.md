...
- Every scenario carries `Label("nic-extra-config", ...)` plus either `"core-functional"` or `"extended-functional"` — the latter for scenarios that exercise DeviceChange fields gated by hardware/firmware prerequisites (`uptv2Enabled`, `vnumaNodeID`) and are more disruptive to set up.
...
- Per `e2e-testing.md`, new/changed E2E scenarios must carry `Label("experimental", ...)` until validated against a real Supervisor; this suite does not yet carry that label on any scenario. Treat that as an open item, not a signal that the suite has already been hardware-validated.
