		...
		Context("With VCSim", func() {
			...
			JustBeforeEach(func() {
				...
				for _, file := range moVM.LayoutEx.File {
					if strings.HasSuffix(file.Name, ".vmdk") ||
						strings.HasSuffix(file.Name, ".vmsn") ||
						strings.HasSuffix(file.Name, ".vmem") {
						sum += file.Size
					}
				}
			})
			...
			It("succeeds", func() {
				total, fcdTotal, err := virtualmachine.GetSnapshotSize(
					vmCtx.Logger, vmCtx.MoVM, moVM.Snapshot.CurrentSnapshot)
				Expect(err).NotTo(HaveOccurred())
			...
			When("the snapshot is nil", func() {
				It("returns 0", func() {
					total, fcdTotal, err := virtualmachine.GetSnapshotSize(vmCtx.Logger, vmCtx.MoVM, nil)
					Expect(err).To(HaveOccurred())
		...
		Context("Mock VM with two snapshots, 1 disk and 1 FCD", func() {
			...
			When("Get the snapshot size of Snapshot-1", func() {
				It("returns the size", func() {
					total, fcdTotal, err := virtualmachine.GetSnapshotSize(
						vmCtx.Logger, vmCtx.MoVM, &snapshot1)
					Expect(err).NotTo(HaveOccurred())
					// 3, 4, 17, 18
			...
			When("Get the snapshot size of Snapshot-2", func() {
				It("returns the size", func() {
					total, fcdTotal, err := virtualmachine.GetSnapshotSize(
						vmCtx.Logger, vmCtx.MoVM, &snapshot2)
					Expect(err).NotTo(HaveOccurred())
					// 5, 6, 19
	...
	Context("GetAllSnapshotSize", func() {
		Context("With VCSim", func() {
			...
			When("the snapshot is nil", func() {
				...
				It("returns 0", func() {
					total, fcdTotal, err := virtualmachine.GetAllSnapshotSize(vmCtx.Logger, vmCtx.MoVM)
					Expect(err).NotTo(HaveOccurred())
					Expect(total).To(BeZero())
					Expect(fcdTotal).To(BeZero())
				})
