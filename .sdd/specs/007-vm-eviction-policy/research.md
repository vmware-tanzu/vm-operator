# Research: VM Eviction Compute Policies

- **Spec**: [`spec.md`](./spec.md)

## Source documents

- WIKI page 2535776102 — "One pager: VM Service - Support for VM Eviction Compute Policies". Business problem, goals/non-goals, design overview, status-condition computation, references (an internal problem-statement doc and a DRS proposal design doc — not reproduced here per this repo's internal-link policy).
- WIKI page 2781824956 — "VM Eviction Compute Policies CRDs". CRD shape and example manifests for both new policy kinds, plus the `VirtualMachine` spec/status examples.
...
- `MatchSpec` (workload labels, guest ID/family, image name/labels, boolean composition) is shared across every policy kind and does not need to change for this feature — both `AutomaticVMEvictionPolicy` and `BestEffortRestartPolicy` reuse it as-is per the example manifests in WIKI page 2781824956.
...
- `VirtualMachinePowerStateSynced` condition already exists (`api/v1alpha5/virtualmachine_types.go:84-86`, mirrored in `v1alpha6`) and is already computed every reconcile in `reconcileStatusPowerState`:
  ...
  ```
- The `InfraInMaintenance` case is detected from the VM's own power-op task failure, with no new vCenter call:
  - `pkg/util/vsphere/vm/power_state.go`'s failure path (`setPowerState`/`doAndWaitOnHardPowerOp`) already has the failed task's `*vimtypes.TaskInfo` in hand at the moment VM Operator's own power-on/off attempt fails — that's where the fault is checked.
  - A new pure helper, `isInfraMaintenanceFault(ti *vimtypes.TaskInfo) bool` (`pkg/util/vsphere/task/task.go`), matches a `*vimtypes.NoCompatibleHost` fault (the one existing type-assertion site for this fault type is `pkg/util/vsphere/fault/fault.go:33`, `LocalizedMessagesFromFault`, for message-extraction purposes only) whose nested fault messages carry the key `com.vmware.cp.autoevac.RestartOnCurrentHostRequired.HostInMaintenanceMode` or `com.vmware.cp.autoevac.RestartOnCurrentHostRequired`.
  - A third, non-autoevac key (`com.vmware.cdrs.hostState.maintenanceMode`) was investigated after a manual repro using host-VM affinity (rather than either autoevac policy fault) produced the same `NoCompatibleHost` shape with this generic cdrs key instead. It was deliberately not adopted: `NoCompatibleHost.Error` carries one entry per candidate host in the cluster, and this key is not scoped to a specific policy or host the way the autoevac keys are, so matching on it without correlating against the VM's own current host risks a false `InfraInMaintenance` signal for a host unrelated to this VM. Doing that correlation correctly would require threading the VM's current host MoRef through the power-op failure path, which was judged not worth the added complexity for this spec; left here as a note in case a future need reopens it. Per DRS team: this is acceptable to not signal on, since this is a best-effort policy — if a VM can't be evacuated/powered on elsewhere for some reason, it's fine because upper layers (e.g. VKS) can act to replace the node. This is inconsistent with `AutomaticVMEvictionPolicy`/BER being treated as planned-maintenance signals, but was left as-is for now.
  - `power_state.go` wraps its returned error with a new sentinel `ErrInfraMaintenanceFault` (via `%w`) when `isInfraMaintenanceFault(ti)` matches, following the existing `pkgerr` sentinel-error convention (`RequeueError`/`NoRequeueError`, matched via `errors.As`/`errors.Is` from any call-stack depth).
  - Per `operator-best-practices.md`, "Reconcile status" (where `reconcileStatusPowerState` runs) happens *before* "Reconcile power state" in the same pass, so the `VirtualMachinePowerStateSynced` **condition** (not the `Status.PowerState` field, which stays in `reconcileStatusPowerState`) is set in `reconcilePowerState` (`pkg/providers/vsphere/vmprovider_vm.go:1494`, the "Reconcile power state" step) instead — it already has `vmCtx.VM` in scope, runs unconditionally every reconcile (including the no-op case where power state already matches), and reflects this pass's actual outcome via `errors.Is(err, ErrInfraMaintenanceFault)` rather than a pre-op snapshot.
- `ReconcileStatusData` (passed through every `reconcileStatus*` function) currently only carries `NetworkDeviceKeysToSpecIdx`, unchanged by this feature.
- `pkg/providers/vsphere/vcenter/host.go` (home of `GetESXHostFQDN`, the only existing host-property-collector helper) is not touched by this feature.
...
- The status-condition branch is gated behind the same single, shared capability/flag (`supports_infrapolicy_vm_evacuation` / `Features.VMEviction`) as both new CRDs — there is no independent rollout of the status signal versus either policy CRD.
