...
func TestNewConditionsGroup(t *testing.T) {
	...
	conditions := []*vmopv1a1.Condition{nil1, true1, true1, falseInfo1, falseWarning1, falseWarning1, falseError1, unknown1}

	got := getConditionGroups(conditionsWithSource(&vmopv1a1.VirtualMachine{}, conditions...))
	...
	g.Expect(got.TopGroup().severity).To(Equal(vmopv1a1.ConditionSeverityError))
	...
	g.Expect(got.TrueGroup().severity).To(Equal(vmopv1a1.ConditionSeverityNone))
	...
	g.Expect(got.ErrorGroup().severity).To(Equal(vmopv1a1.ConditionSeverityError))
	...
	g.Expect(got.WarningGroup().severity).To(Equal(vmopv1a1.ConditionSeverityWarning))
	...
	g.Expect(got[0].severity).To(Equal(vmopv1a1.ConditionSeverityError))
	...
	g.Expect(got[1].severity).To(Equal(vmopv1a1.ConditionSeverityWarning))
	...
	g.Expect(got[2].severity).To(Equal(vmopv1a1.ConditionSeverityInfo))
	...
	g.Expect(got[3].severity).To(Equal(vmopv1a1.ConditionSeverityNone))
	...
	g.Expect(got[4].severity).To(Equal(vmopv1a1.ConditionSeverityNone))
...
func TestMergeRespectPriority(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			conditions: []*vmopv1a1.Condition{falseError1, falseWarning1, falseInfo1, unknown1, true1},
			want:       FalseCondition("foo", "reason falseError1", vmopv1a1.ConditionSeverityError, "message falseError1"),
		},
		{
			name:       "When there is false/warning and no false/error, it returns false/warning",
			conditions: []*vmopv1a1.Condition{falseWarning1, falseInfo1, unknown1, true1},
			want:       FalseCondition("foo", "reason falseWarning1", vmopv1a1.ConditionSeverityWarning, "message falseWarning1"),
		},
		{
			name:       "When there is false/info and no false/error or false/warning, it returns false/info",
			conditions: []*vmopv1a1.Condition{falseInfo1, unknown1, true1},
			want:       FalseCondition("foo", "reason falseInfo1", vmopv1a1.ConditionSeverityInfo, "message falseInfo1"),
		},
		{
			name:       "When there is true and no false/*, it returns info",
			conditions: []*vmopv1a1.Condition{unknown1, true1},
			want:       TrueCondition("foo"),
		},
		{
			name:       "When there is unknown and no true or false/*, it returns unknown",
			conditions: []*vmopv1a1.Condition{unknown1},
			want:       UnknownCondition("foo", "reason unknown1", "message unknown1"),
		},
		{
			name:       "nil conditions are ignored",
			conditions: []*vmopv1a1.Condition{nil1, nil1, nil1},
			want:       nil,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := merge(conditionsWithSource(&vmopv1a1.VirtualMachine{}, tt.conditions...), "foo", &mergeOptions{})
