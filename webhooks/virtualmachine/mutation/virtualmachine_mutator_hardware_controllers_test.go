...
func testSetPVCVolumesDefaults(getCtx func() *unitMutationWebhookContext) {
	Describe("SetPVCVolumeDefaults", func() {
		var (
		...
		JustBeforeEach(func() {
			wasMutated, err = mutation.SetPVCVolumeDefaults(&ctx.WebhookRequestContext, ctx.Client, vm)
		})
...
func testSchemaUpgradeCheck(getCtx func() *unitMutationWebhookContext) {
	Describe("Schema Upgrade Check", func() {
		...
		It("should not add controllers when VM schema is not upgraded", func() {
			...
			// Verify controller was NOT added (because VMSchemaUpgraded returns false)
		...
		It("should not set PVC defaults when VM schema is not upgraded", func() {
			...
			// Verify PVC defaults were NOT set (because VMSchemaUpgraded returns false)
