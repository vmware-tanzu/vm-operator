...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	defer func() {
		...
		setSnapshotReadyCondition(vmSnapshot)
...
func (r *Reconciler) ReconcileNormal(ctx *pkgctx.VirtualMachineSnapshotContext) (ctrl.Result, error) {
	...
	// Only start calculating the used capacity if the snapshot is created.
	if !pkgcnd.IsTrue(vmSnapshot, vmopv1.VirtualMachineSnapshotCreatedCondition) {
		return ctrl.Result{}, nil
	}
...
func setSnapshotReadyCondition(vmSnapshot *vmopv1.VirtualMachineSnapshot) {
	// If the snapshot is being deleted, skip setting the condition.
	if !vmSnapshot.DeletionTimestamp.IsZero() {
		return
	}

	if pkgcnd.Get(vmSnapshot, vmopv1.VirtualMachineSnapshotCreatedCondition) == nil {
		pkgcnd.MarkFalse(vmSnapshot,
			vmopv1.VirtualMachineSnapshotCreatedCondition,
			vmopv1.VirtualMachineSnapshotCreationPendingReason,
			"Snapshot is pending to be created")
	}

	if pkgcnd.Get(vmSnapshot, vmopv1.VirtualMachineSnapshotCSIVolumeSyncedCondition) == nil {
		pkgcnd.MarkFalse(vmSnapshot,
			vmopv1.VirtualMachineSnapshotCSIVolumeSyncedCondition,
			vmopv1.VirtualMachineSnapshotCSIVolumeSyncPendingReason,
			"CSI volume sync is pending to be requested")
	}
