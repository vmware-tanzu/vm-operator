		...
		When("Nested Snapshot, one of them is marked for deletion", func() {
			...
			When("internal snapshot is deleted", func() {
				...
				It("returns success, vm current snapshot is updated to root", func() {
					...
					Expect(vmSnapshotL1Obj.Status.Children).To(ContainElement(*vmSnapshotCRToManagedSnapshotRefWithDefaultVersion(vmSnapshotL3Node1)))
					Expect(vmSnapshotL1Obj.Status.Children).To(ContainElement(*vmSnapshotCRToManagedSnapshotRefWithDefaultVersion(vmSnapshotL3Node2)))
				})
			})
