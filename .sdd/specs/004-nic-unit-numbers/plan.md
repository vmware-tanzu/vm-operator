...
- **E2E tests** (mandatory — cluster-observable; `test/e2e/vmservice/vmservice/virtualmachine/` — note double `vmservice`):
  - New file `vm_nic_unit_numbers.go` with `func VMNICUnitNumbersSpec(...)` following the `vm_networking.go` pattern.
  - Create VM without explicit `unitNumber` → after first reconcile, backfill has recorded vSphere-assigned values (starting at 7) into spec; status matches.
  - Create VM with explicit `unitNumber` → device is placed at the requested slot; spec value unchanged; status matches.
  - Add NIC to running VM → new `unitNumber` assigned and unique.
  - Remove NIC → correct device removed; remaining NIC retains its unit number in status.
  - Submit VM with duplicate `unitNumber` → webhook rejects.
