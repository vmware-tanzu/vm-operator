...
// https://github.com/kubernetes-sigs/cluster-api/blob/0008b5ba109e839067b96f4c5b52eab9fb657eca/test/e2e/common.go
package framework
...
func WatchNamespaceEvents(ctx context.Context, input WatchNamespaceEventsInput) {
	...
	_, err = eventInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj any) {
			e := obj.(*corev1.Event)
			_, _ = fmt.Fprintf(f, "[New Event] %s/%s\n\tresource: %s/%s/%s\n\treason: %s\n\tmessage: %s\n\tfull: %#v\n",
				e.Namespace, e.Name, e.InvolvedObject.APIVersion, e.InvolvedObject.Kind, e.InvolvedObject.Name, e.Reason, e.Message, e)
		},
		UpdateFunc: func(_, obj any) {
			e := obj.(*corev1.Event)
			_, _ = fmt.Fprintf(f, "[Updated Event] %s/%s\n\tresource: %s/%s/%s\n\treason: %s\n\tmessage: %s\n\tfull: %#v\n",
				e.Namespace, e.Name, e.InvolvedObject.APIVersion, e.InvolvedObject.Kind, e.InvolvedObject.Name, e.Reason, e.Message, e)
		},
		DeleteFunc: func(obj any) {},
	})
	Expect(err).NotTo(HaveOccurred(), "failed to add event handler for namespace %s", input.Name)
...
func CreateNamespace(ctx context.Context, input CreateNamespaceInput, intervals ...any) *corev1.Namespace {
	...
	framework.Logf("Creating namespace %s", input.Name)
	Eventually(func() error {
		err := input.Creator.Create(ctx, ns)
		if err != nil {
...
func CreateNamespaceAndWatchEvents(ctx context.Context, input CreateNamespaceAndWatchEventsInput) (*corev1.Namespace, context.CancelFunc) {
	...
	framework.Logf("Creating event watcher for namespace %q", input.Name)
...
func DeleteNamespace(ctx context.Context, input DeleteNamespaceInput, intervals ...any) {
	Expect(ctx).NotTo(BeNil(), "ctx is required for DeleteNamespace")
	...
	framework.Logf("Deleting namespace %s", input.Name)
	Eventually(func() error {
