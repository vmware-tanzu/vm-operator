...
func (v validator) validateImmutableVMNameLabel(
	...
	if oldVMSnapshot != nil && oldVMSnapshot.Labels[vmopv1.VMNameForSnapshotLabel] != "" {
		// Since we are only validating this during
		// Update, the new VM is always guaranteed to have
		// a non nil label with the VM name set.
