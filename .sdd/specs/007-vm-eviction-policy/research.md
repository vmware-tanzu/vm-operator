...
- The `InfraInMaintenance` case is detected from the VM's own power-op task failure, with no new vCenter call:
  ...
  - A new pure helper, `isInfraMaintenanceFault(ti *vimtypes.TaskInfo) bool` (`pkg/util/vsphere/task/task.go`), matches a `*vimtypes.NoCompatibleHost` fault (the one existing type-assertion site for this fault type is `pkg/util/vsphere/fault/fault.go:33`, `LocalizedMessagesFromFault`, for message-extraction purposes only) whose nested fault messages carry the key `com.vmware.cp.autoevac.RestartOnCurrentHostRequired.HostInMaintenanceMode`, `com.vmware.cp.autoevac.RestartOnCurrentHostRequired`, or `com.vmware.cdrs.hostState.maintenanceMode`.
  - The third key was added after manual verification against a real host-in-maintenance-mode repro that used host-VM affinity (rather than either autoevac policy fault) to make the host the VM's only compatible one; the `NoCompatibleHost`/nested-fault-message shape matched exactly, but the key was the generic cdrs one, not an autoevac key — see PR discussion for the observed task error. Unlike the autoevac keys, this key is not gated to eviction/restart-policy scenarios, so it also fires when host maintenance mode blocks power-on for any other reason a VM is pinned to that host. This is intentional: the signal VKS needs ("don't remediate, it's just host maintenance") applies the same regardless of what pinned the VM there.
