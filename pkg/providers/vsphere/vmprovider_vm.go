...
var (
	...
	ErrMsgNoSnapshots         = "no snapshots for this VM"
	ErrMsgSnapshotNotFound    = "not found"
)
...
func (vs *vSphereVMProvider) reconcileSnapshotRevert(
	...
	snapObj, err := vs.findDesiredSnapshot(vmCtx, vcVM, desiredSnapshotName)
	if err != nil {
	...
	// registration.  Once we do that, we will have to remove that ExtraCoonfig key.
...
func (vs *vSphereVMProvider) findDesiredSnapshot(
	...
	snapObj, err := vcVM.FindSnapshot(vmCtx, snapshotName)
	if err != nil {
		...
		if err.Error() == ErrMsgNoSnapshots {
			vmCtx.Logger.V(4).Info("VM has no snapshots, skipping",
				"snapshotName", snapshotName)
			return nil, nil
		}

		if err.Error() == ErrMsgSnapshotNotFound {
		...
		vmCtx.Logger.Error(err, "Error finding snapshot",
			"snapshotName", snapshotName)
		return nil, pkgerr.NoRequeueNoErr(err.Error())
	}
...
func (vs *vSphereVMProvider) getVMYamlFromSnapshot(
	...
	if moSnap.Config.ExtraConfig == nil {
		return "", fmt.Errorf("VM snapshot does not have VM spec and metadata")
	}
...
func (vs *vSphereVMProvider) reconcileSnapshot(
	...
	// Fetch the VirtualMachineSnapshot object so we have the
	vmSnapshot, err := getVirtualMachineSnapShotObject(vmCtx, vs.k8sClient)
	if err != nil {
		return err
	}

	// Check if a snapshot with the desired name already exists on the VM.
	snapObj, _ := vcVM.FindSnapshot(vmCtx, vmCtx.VM.Spec.CurrentSnapshot.Name)
	if snapObj == nil {
		...
		snapObj, err = virtualmachine.SnapshotVirtualMachine(snapArgs)
		if err != nil {
