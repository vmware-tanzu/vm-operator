...
func CalculateReservedForSnapshotPerStorageClass(
	...
	for _, volume := range vm.Status.Volumes {
		...
		case vmopv1.VirtualMachineStorageDiskTypeClassic:
			...
			logger.V(5).Info("adding disk size to the total reserved capacity", "disk", volume.Name, "storageClass", vm.Spec.StorageClass, "requested", volume.Requested)
		...
		case vmopv1.VirtualMachineStorageDiskTypeManaged:
			...
			if pvcSpec.InstanceVolumeClaim != nil {
				...
				requestedMap[vm.Spec.StorageClass].Add(pvcSpec.InstanceVolumeClaim.Size)
				continue
			}
			...
			volumeRequested := volume.Requested
			// If the requested is not set on volume, then use the PVC.spec.resources.requests.
			if volumeRequested == nil {
				if pvc.Spec.Resources.Requests == nil {
					return nil, fmt.Errorf("requested is not set for managed disk")
				}
				logger.V(5).Info("using PVC.spec.resources.requests for disk", "pvc", pvc.Name, "storageClass", pvc.Spec.StorageClassName)
			...
			logger.V(5).Info("adding disk size to the total reserved capacity", "disk", volume.Name, "storageClass", *pvc.Spec.StorageClassName, "requested", volumeRequested)
