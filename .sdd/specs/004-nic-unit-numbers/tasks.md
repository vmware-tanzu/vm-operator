...
**NOTE on E2E file location**: The correct path is `test/e2e/vmservice/vmservice/virtualmachine/` (double `vmservice`). E2E specs are named `vm_<feature>.go` and use a `func VMXxxSpec(ctx, inputGetter)` pattern. Labels are applied by the suite runner, not inside the spec files. Tests use `skipper.SkipUnlessInfraIs(...)` for infra gating.

- [ ] T022 [vmop-TBD] Create `test/e2e/vmservice/vmservice/virtualmachine/vm_nic_unit_numbers.go` (package `virtualmachine`) with `func VMNICUnitNumbersSpec(ctx context.Context, inputGetter func() VMNICUnitNumbersSpecInput)` following the `vm_networking.go` pattern; cover:
  - Create VM with no explicit `unitNumber` → verify the spec is unchanged at admission (no webhook assignment), then after first reconcile the backfill has recorded vSphere-assigned values (range 7–16, starting at 7, unique) into `spec.network.interfaces[i].unitNumber`, and `status.network.interfaces[i].unitNumber` matches the observed vSphere `VirtualDevice.UnitNumber`
  - Create VM with one explicit `unitNumber: 9` → verify the device is placed at slot 9, the spec value is unchanged, other interfaces receive vSphere-assigned values recorded by the backfill, status matches
  - Add a NIC to a running VM (if allowed) → verify new `unitNumber` is unique and appears in status
  - Remove a NIC from a VM → verify the correct vSphere device was removed (by checking remaining status `unitNumber` values), remaining NIC retains its unit number
  - Submit a VM with duplicate `unitNumber` values → verify admission webhook rejects with the expected `field.Invalid` error on `spec.network.interfaces[i].unitNumber`
...
```
T001 (research) ─────────────────────────────────► T018, T019, T029 (Add/Edit/create payload decisions)
...
| T022 | `test/e2e/vmservice/vmservice/virtualmachine/vm_nic_unit_numbers.go` (new) |
