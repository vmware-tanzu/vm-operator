...
func (s *Session) poweredOnReconfigure(
	...
	if errors.Is(err, ErrReconfigure) {
		if cbtErr := s.reconcilePoweredOnChangeBlockTracking(vmCtx, configSpec); cbtErr != nil {
			...
			// so continue to do that here for now. That the reconfigure was not just handle
			// this is a broken VC API. A few options for later:
			// - Try to un Reconfigure CBT so we'll retry on the next reconcile
			// - Annotation the VM object, and hope the k8s update succeeds, to indicate
