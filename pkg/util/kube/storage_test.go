...
var _ = Describe("IsEncryptedStorageProfile", func() {
	var (
		...
		storageClass  storagev1.StorageClass
		storagePolicy polv1.StoragePolicy
		profileID     string
	)
...
var _ = Describe("MarkEncryptedStorageClass", func() {
	...
	BeforeEach(func() {
		...
		funcs = interceptor.Funcs{
			...
			Patch: func(
				...
				var savedStatus *polv1.StoragePolicyStatus
				if pol, ok := obj.(*polv1.StoragePolicy); ok {
	...
	JustBeforeEach(func() {
		...
		client = fake.NewClientBuilder().
			WithObjects(withObjs...).
			WithScheme(scheme).
			WithStatusSubresource(&polv1.StoragePolicy{}).
			WithInterceptorFuncs(funcs).
			Build()
	...
	When("creating the StoragePolicy returns an error", func() {
		BeforeEach(func() {
			funcs.Create = func(
				...
				if _, ok := obj.(*polv1.StoragePolicy); ok {
					return apierrors.NewInternalError(errors.New(fakeString))
				}
