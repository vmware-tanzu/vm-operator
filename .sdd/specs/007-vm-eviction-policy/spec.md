...
- MUST add a new, non-host-identifying explanation to the existing `VirtualMachinePowerStateSynced` condition: when VM Operator's own attempt to converge `spec.powerState` fails because the underlying vSphere power-state-change task returns a `NoCompatibleHost` fault whose nested fault messages carry the key `com.vmware.cp.autoevac.RestartOnCurrentHostRequired.HostInMaintenanceMode`, `com.vmware.cp.autoevac.RestartOnCurrentHostRequired`, or `com.vmware.cdrs.hostState.maintenanceMode`, set the condition `False` with reason `InfraInMaintenance` instead of the generic `NotSynced` reason. The third key is not autoevac/policy-specific — it appears on any `NoCompatibleHost` fault where host maintenance mode is what makes the VM's only viable host incompatible (e.g. a plain host-VM affinity constraint), and is included because it signals the same "don't remediate, it's just host maintenance" condition to VKS regardless of which mechanism pinned the VM to that host. No host state is queried directly — the signal comes from the failed task's own fault, not a separate host-status lookup. The condition returns to its normal computation once a subsequent convergence attempt no longer hits this fault (i.e. the host has exited maintenance mode and the VM powers back on).
