...
The feature is gated on `Features.TaggingAPI`, which is gated behind the Supervisor capability `PlacementPoliciesForVMServiceVmsV3` (spec G13). That capability mapping is not yet wired into `pkg/config/capabilities/capabilities.go` in this change set, so there is no capability for the suite to query yet. Until it lands, the suite:
...
- Should switch to a capability lookup with a named constant (e.g. `PlacementPoliciesForVMServiceVmsV3CapabilityName`) in `test/e2e/vmservice/consts/consts.go` — alongside the existing `VMAffinityDuringExecutionCapabilityName` — once `PlacementPoliciesForVMServiceVmsV3` is wired (`research.md` "Open follow-ups"). That closes the one-way gap; T037 should not drop `"experimental"` before it does.
...
`spec.affinity` is immutable (spec NG8), so these scenarios change a running VM's **labels**. Ownership follows the affinity reference alone, so dropping a label never drops ownership — the VM's `spec.affinity` still references the pair and cannot change — only that VM's own vCenter tag carriage reacts.
