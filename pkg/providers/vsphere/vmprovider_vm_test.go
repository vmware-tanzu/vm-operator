	...
	Context("Create/Update/Delete VirtualMachine", func() {
		...
		Context("CreateOrUpdate VM", func() {
			...
			Context("Snapshot Revert", func() {
				...
				// Simulate an ImportedVM scenario by creating the VC VM and VM while ensuring the backup is not taken.
				// Then,
				//	- creating a snapshot on VC AND THEN only creating the VMSnapshot CR so that the ExtraConfig is
				// 	  not taken by the controller.
				...
				Context("when reverting to imported snapshot", func() {
					...
					It("should successfully revert to desired snapshot and approximate the VM Spec", func() {
						// Annotate as an ImportedVM
						annotations := make(map[string]string)
						annotations[vmopv1.ImportedVMAnnotation] = ""
						...
						// so that the update process infers the VMClass from ConfigInfo
						vm.Spec.ClassName = ""
						...
						// Verify the revert operation reverted to the expected value
