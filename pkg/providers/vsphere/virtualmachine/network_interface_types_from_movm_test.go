...
func networkInterfaceTypeTests() {
	...
	Describe("FillEmptyNetworkInterfaceTypesFromMoVM", func() {
		...
		When("moVM has mixed NIC types with some Type already set", func() {
			BeforeEach(func() {
				vm.Spec.Network.Interfaces = []vmopv1.VirtualMachineNetworkInterfaceSpec{
					{Name: "eth0", Type: vmopv1.VirtualMachineNetworkInterfaceTypeE1000},
					{Name: "eth1"},
					{Name: "eth2", Type: vmopv1.VirtualMachineNetworkInterfaceTypePCNet32},
				}
				moVM = moVMWithEthernet(
					&vimtypes.VirtualE1000{},
					&vimtypes.VirtualE1000e{},
			...
			It("should set correct type for each interface", func() {
				expectMutationSuccess()
				Expect(vm.Spec.Network.Interfaces[0].Type).To(Equal(vmopv1.VirtualMachineNetworkInterfaceTypeE1000))
				Expect(vm.Spec.Network.Interfaces[1].Type).To(Equal(vmopv1.VirtualMachineNetworkInterfaceTypeE1000e))
				Expect(vm.Spec.Network.Interfaces[2].Type).To(Equal(vmopv1.VirtualMachineNetworkInterfaceTypePCNet32))
			})
		})
