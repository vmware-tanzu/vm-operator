...
func (vs *vSphereVMProvider) restoreVMSpecFromSnapshot(
	...
	if err := yaml.Unmarshal([]byte(vmYAML), &vm); err != nil {
		return fmt.Errorf(
			"failed to unmarshal snapshot payload into vm: %w", err)
	}
...
func (vs *vSphereVMProvider) reconcileCurrentSnapshot(
	...
	if totalPendingSnapshots > 0 {
		return fmt.Errorf("requeuing to process %d remaining snapshots: %w",
			totalPendingSnapshots, pkgerr.RequeueError{})
	}
