...
func (r *Reconciler) ReconcileNormal(
	...
	opRes, copErr := controllerutil.CreateOrPatch(
		...
		func() error {
			...
			// Clear the status so it can be rebuilt.
			*vmiStatus = vmopv1.VirtualMachineImageStatus{}
...
func (r *Reconciler) syncImageContent(
	...
	if err != nil {
		...
		if labels := cliObj.GetLabels(); labels != nil {
			// Now that the VMI is synced, the CLI no longer needs to be
			// reconciled when the VMI Cache object is updated.
