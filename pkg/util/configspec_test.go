...
var _ = Describe("MergeStorageControllersAndDisks", func() {
	...
	When("both src and dst are empty", func() {
		It("succeeds with no changes", func() {
			err := pkgutil.MergeStorageControllersAndDisks(dst, src, "")
			Expect(err).ToNot(HaveOccurred())
	...
	When("src has controllers and disks", func() {
		...
		It("applies storage policy to disks", func() {
			err := pkgutil.MergeStorageControllersAndDisks(dst, src, "test-policy-id")
			Expect(err).ToNot(HaveOccurred())
			Expect(dst.DeviceChange).To(HaveLen(2))
...
var _ = Describe("FullyDefinedControllersToConfigSpecs", func() {
	var (
		ctx        context.Context
	...
	When("vm is nil", func() {
		It("returns empty config spec", func() {
			result := pkgutil.FullyDefinedControllersToConfigSpecs(ctx, nil, configSpec)
	...
	When("vm.Spec.Hardware is nil", func() {
		...
		It("returns empty config spec", func() {
			result := pkgutil.FullyDefinedControllersToConfigSpecs(ctx, vm, configSpec)
			Expect(result.DeviceChange).To(BeEmpty())
		})
	})
