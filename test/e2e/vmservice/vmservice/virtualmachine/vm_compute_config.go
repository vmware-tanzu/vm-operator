...
func VMComputeConfigSpec(ctx context.Context, inputGetter func() VMComputeConfigSpecInput) {
	...
	It("creates VM with hot-pluggable allocation and verifies condition True after live update",
		Label("core-functional", "experimental"), func() {
			...
			vm := buildComputeConfigVM(vmName, vmNamespace, vmClassName, linuxVMIName, storageClass,
				&vmopv1.VirtualMachineResourcesSpec{
	...
	It("sets PowerOffRequired condition for multiple power-off-required fields while VM is powered on",
		Label("core-functional", "experimental"), func() {
			...
			latest.Spec.MemoryAdvanced = &vmopv1.VirtualMachineMemoryAdvancedSpec{
				HotAddEnabled: ptr.To(true),
			}
			Expect(svClusterClient.Patch(ctx, latest, patch)).To(Succeed(),
				"failed to patch VirtualMachine %s/%s", vmNamespace, vmName)
		...
		Label("core-functional", "experimental"), func() {
			...
			makeVM := func() *vmopv1.VirtualMachine {
				return buildComputeConfigVM(vmName, vmNamespace, vmClassName, linuxVMIName, storageClass,
					...
					&vmopv1.VirtualMachineCPUAdvancedSpec{
						HotAddEnabled: ptr.To(true),
					},
					nil,
				)
			...
			By("Deleting VM")
			vmToDelete, err := utils.GetVirtualMachine(ctx, svClusterClient, vmNamespace, vmName)
			Expect(err).ToNot(HaveOccurred())
	...
	It("re-applies compute config fields after vSphere VM is destroyed out-of-band",
		Label("extended-functional", "experimental"), func() {
			...
			latest, err := utils.GetVirtualMachine(ctx, svClusterClient, vmNamespace, vmName)
			Expect(err).ToNot(HaveOccurred())
			latestPatch := latest.DeepCopy()
			latestPatch.Spec.PowerState = vmopv1.VirtualMachinePowerStateOn
			Expect(svClusterClient.Patch(ctx, latestPatch, ctrlclient.MergeFrom(latest))).To(Succeed(),
			...
			Eventually(func(g Gomega) {
				latest, getErr := utils.GetVirtualMachine(ctx, svClusterClient, vmNamespace, vmName)
	...
	It("reverts live compute config drift injected directly in vSphere",
		Label("extended-functional", "experimental"), func() {
			...
			vmBeforeDrift, err := utils.GetVirtualMachine(ctx, svClusterClient, vmNamespace, vmName)
			Expect(err).ToNot(HaveOccurred())
	...
	It("sets latency sensitivity High with full CPU and memory reservation",
		Label("extended-functional", "experimental"), func() {
			...
			Expect(err).ToNot(HaveOccurred())
			patch := ctrlclient.MergeFrom(latest.DeepCopy())
			latLevel := vmopv1.VirtualMachineLatencySensitivityHigh
			latest.Spec.CPUAdvanced = &vmopv1.VirtualMachineCPUAdvancedSpec{
	...
	It("applies vnumaNodeCount topology via power-off reconfigure on vmx-20+ VM",
		Label("extended-functional", "experimental"), func() {
			...
			// An explicit (non-auto) CoresPerNumaNode is reflected in
			// config.numaInfo immediately on Reconfigure, even while the VM
			// is powered off; it is not runtime-only. Verify it both before
			// and after power-on so a regression to power-state-dependent
			// behavior is caught either way.
			verifyCoresPerNumaNode := func(when string) {
				By(fmt.Sprintf("Verifying coresPerNumaNode via govmomi (%s)", when))
				Eventually(func(g Gomega) {
					...
					_, _ = fmt.Fprintf(GinkgoWriter,
						"[cc-vnuma] %s RetrieveOne: err=%v config=%v numaInfo=%v\n",
						when, err, moVM.Config != nil,
					...
					_, _ = fmt.Fprintf(GinkgoWriter,
						"[cc-vnuma] %s coresPerNumaNode=%d\n",
						when, *moVM.Config.NumaInfo.CoresPerNumaNode)
					...
					g.Expect(*moVM.Config.NumaInfo.CoresPerNumaNode).To(BeEquivalentTo(int32(1)),
						"expected coresPerNumaNode=1 (2 vCPUs / vnumaNodeCount=2) (%s)", when)
				}, config.GetIntervals("default", "wait-vm-compute-config-synced")...).Should(Succeed(),
					"timed out waiting for vNUMA config to be applied in vSphere for %s/%s (%s)",
					vmNamespace, vmName, when)
			}

			verifyCoresPerNumaNode("powered off")

			By("Powering on VM to confirm coresPerNumaNode persists across the power cycle")
			vmoperator.UpdateVirtualMachinePowerState(ctx, config, svClusterClient, vmNamespace, vmName, string(vmopv1.VirtualMachinePowerStateOn))
			vmoperator.WaitForVirtualMachinePowerState(ctx, config, svClusterClient,
				vmNamespace, vmName, string(vmopv1.VirtualMachinePowerStateOn))
	...
	It("applies spec CPU and memory size overrides while powered off and verifies after power-on",
		Label("core-functional", "experimental"), func() {
			...
			vm := buildComputeConfigVM(vmName, vmNamespace, vmClassName, linuxVMIName, storageClass,
				...
				&vmopv1.VirtualMachineMemoryAdvancedSpec{
					HotAddEnabled: ptr.To(true),
				},
			)
			vm.Spec.PowerState = vmopv1.VirtualMachinePowerStateOff
			...
			latest.Spec.Resources.Size = &vmopv1.VirtualMachineResourceQuantity{
...
func waitForComputeConfigSynced(
	...
	Eventually(func(g Gomega) {
		vm, err := utils.GetVirtualMachine(ctx, svClient, key.Namespace, key.Name)
		g.Expect(err).ToNot(HaveOccurred())
		cond := getComputeConfigCondition(vm)
		g.Expect(cond).ToNot(BeNil(),
...
func statusCPU(vm *vmopv1.VirtualMachine) *vmopv1.VirtualMachineCPUAllocationStatus {
...
func statusMemory(vm *vmopv1.VirtualMachine) *vmopv1.VirtualMachineMemoryAllocationStatus {
	if vm == nil || vm.Status.Hardware == nil {
		return nil
	}
...
func getComputeConfigCondition(vm *vmopv1.VirtualMachine) *metav1.Condition {
	if vm == nil {
		return nil
	}
	for i := range vm.Status.Conditions {
		c := &vm.Status.Conditions[i]
		if c.Type == vmopv1.VirtualMachineConditionComputeConfigSynced {
			return c
		}
	}
	return nil
...
func getClassConfigCondition(vm *vmopv1.VirtualMachine) *metav1.Condition {
	if vm == nil {
		return nil
	}
	for i := range vm.Status.Conditions {
		c := &vm.Status.Conditions[i]
		if c.Type == vmopv1.VirtualMachineClassConfigurationSynced {
			return c
		}
	}
	return nil
...
// buildComputeConfigVM constructs a vmopv1.VirtualMachine with the given
...
func buildComputeConfigVM(
	...
	cpuAdv *vmopv1.VirtualMachineCPUAdvancedSpec,
	memAdv *vmopv1.VirtualMachineMemoryAdvancedSpec,
...
func injectAllocationDrift(
	...
	if err := task.Wait(ctx); err != nil {
		return fmt.Errorf("failed to wait for reconfigure task: %w", err)
	}
	return nil
}
