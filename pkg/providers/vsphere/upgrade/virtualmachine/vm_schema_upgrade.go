...
func reconcileVirtualDisks(
	...
	if vm.Status.BiosUUID == "" {
		logger.V(4).Info("Skipping disks reconciliation due to BiosUUID not available")
		return
	}
...
func buildPVCNameToDiskUUIDMap(
	...
	for _, attm := range attachments {
		...
		if attm.Status.Attached && diskUUID != "" && pvcName != "" {
...
func reconcileCdromSpec(
	...
	if err != nil {
		logger.Error(err, "Error getting backing file name")
		return
	}
