...
type ConfigTargetDevices struct {
	...
	// passthrough devices.
	SCSIPassthrough []VirtualMachineScsiPassthroughInfo `json:"scsiPassthrough,omitempty"`
...
type VirtualMachinePCIPassthroughInfo struct {
	...
	// ID is composed of systemID:pciDevice.id.
	ID string `json:"id"`
...
type VirtualMachineSriovInfo struct {
	...
	// virtual function instantiated by an SR-IOV capable device.
...
// VirtualMachineScsiPassthroughInfo describes a generic SCSI device, including
...
type VirtualMachineScsiPassthroughInfo struct {
	...
	// PhysicalUnitNumber is the unit number of the generic device on the
	// physical host.
	PhysicalUnitNumber int32 `json:"physicalUnitNumber"`

	// +required

	// SCSIClass is the class of the SCSI passthrough device.
	SCSIClass string `json:"scsiClass"`

	// +required

	// Vendor is the vendor of the SCSI passthrough device.
