...
func (v validator) validateVMAffinity(
	...
	if a := affinity.VMAffinity; a != nil {
		...
		if rs := a.RequiredDuringSchedulingIgnoredDuringExecution; len(rs) > 0 {
			...
			for idx, rs := range rs {
				...
				if rs.LabelSelector != nil {
					if len(rs.LabelSelector.MatchExpressions) != 0 {
						allErrs = append(allErrs, field.Forbidden(
							p.Child("labelSelector", "matchExpressions"), "MatchExpressions is not allowed"))
					}
				}
		...
		if ps := a.PreferredDuringSchedulingIgnoredDuringExecution; len(ps) > 0 {
			allErrs = append(allErrs, field.Forbidden(p.Child("preferredDuringSchedulingIgnoredDuringExecution"),
				"VM affinity with PreferredDuringSchedulingIgnoredDuringExecution is not allowed"))
		}
	}
	...
	if a := affinity.VMAntiAffinity; a != nil {
				...
				if rs.LabelSelector != nil {
					...
					for exprIdx, expr := range rs.LabelSelector.MatchExpressions {
						if expr.Operator != metav1.LabelSelectorOpIn {
							allErrs = append(allErrs, field.NotSupported(
								p.Child("matchExpressions").Index(exprIdx).Child("operator"),
								expr.Operator,
		...
		if rs := a.RequiredDuringSchedulingPreferredDuringExecution; len(rs) > 0 {
		...
		if ps := a.PreferredDuringSchedulingPreferredDuringExecution; len(ps) > 0 {
