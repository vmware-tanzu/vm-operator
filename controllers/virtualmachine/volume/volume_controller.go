...
func (r *Reconciler) processAttachments(
	...
	for _, volume := range ctx.VM.Spec.Volumes {
		...
		// Must mark this as true now even if something below fails in order to try to keep the
		// volumes attached in order.
...
func (r *Reconciler) handlePVCWithWFFC(
	...
	if pvc.Annotations[constants.KubernetesSelectedNodeAnnotationKey] != "" {
		...
		// TBD: Make check if the Node still exists, and is in the VM's Zone?
		return nil
	}
	...
	if scName == nil || *scName == "" {
		...
		// trying to resolve that here now and just continue the prior flow.
		return nil
	}
