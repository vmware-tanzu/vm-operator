...
24. **`status.readyReplicas` tracks the owned VMs' `Ready` condition, and treats a VM without a configured readiness probe as implicitly ready**
    ...
    **RESOLVED (supersedes prior wording)**: a `VirtualMachine`'s `Ready` condition is only ever populated by the readiness prober (`pkg/prober`), and the prober only watches VMs that have `spec.readinessProbe` configured with at least one of `tcpSocket`/`guestHeartbeat`/`guestInfo` (`prober_manager.go`'s `AddToProberManager`). A VM created without a readiness probe therefore never gets a `Ready` condition at all, by design — treating "condition absent" as "not ready" for that case would mean `status.readyReplicas` can never reach `status.replicas` for any `VirtualMachineReplicaSet` whose template omits `readinessProbe` (the common case), regardless of whether the VMs are actually up. This mirrors a real gap Pod `ReplicaSet` does not have: kubelet marks a container ready as soon as it starts when no probe is configured, so an absent probe means "ready", not "unknown forever". `status.readyReplicas` adopts the same rule: a VM counts as ready if its `Ready` condition is `True`, **or** if it has no `spec.readinessProbe` configured (implicitly ready, condition absent). A VM *with* a configured probe but whose `Ready` condition has not yet been set (or is `False`) does **not** count as ready — the prober is expected to run and populate it. This is the same convention already used by the `VirtualMachineService` controller's endpoint-readiness logic (`virtualMachineHasReadinessProbe` in `virtualmachineservice_controller.go`), extended here to `readyReplicas`.
