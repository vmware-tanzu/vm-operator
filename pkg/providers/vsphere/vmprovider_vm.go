...
func (vs *vSphereVMProvider) reconcileSnapshotRevert(
	...
	// Log a message if the VM already points to the current snapshot, but new change
	// might be added to the current VM, so we want to continue with the revert.
	if vmCtx.MoVM.Snapshot != nil && vmCtx.MoVM.Snapshot.CurrentSnapshot != nil &&
		vmCtx.MoVM.Snapshot.CurrentSnapshot.Value == snapObj.Reference().Value {
		vmCtx.Logger.Info("VM already points to the current snapshot, but new change might be " +
			"added to the current VM. Revert to the original snapshot anyway")
	}
