...
func (v validator) validateBootOrder(
	...
	if create {
		...
		// validate that the device type is one of the valid options and that
		// the device exists.
		for _, bd := range vm.Spec.BootOptions.BootOrder {
			switch bd.Type {
				...
				if !found {
					allErrs = append(
						...
						field.Invalid(
							fieldPath,
							bd,
							fmt.Sprintf("bootable network device %q was not found in the list of network interfaces for this VM", bd.Name),
						))
				}
				...
				if !found {
					allErrs = append(
						...
						field.Invalid(
							fieldPath,
							bd,
							fmt.Sprintf("bootable disk device %q was not found in the list of volumes for this VM", bd.Name),
						))
				}
			...
			default:
				allErrs = append(
					...
					field.Invalid(
						fieldPath,
						bd,
						fmt.Sprintf("unsupported bootable device type: %s", bd.Type),
					))
			}
		}
	}
