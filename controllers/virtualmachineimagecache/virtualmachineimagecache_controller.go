...
var endsWithVMDK = regexp.MustCompile(`(?i)^.+\.vmdk$`)
...
func (r *reconciler) cacheDisks(
	...
	cachedPaths, err := clsutil.CacheStorageURIs(
		ctx,
		sriClient,
		dstDatacenter,
		srcDatacenter,
...
func getDatastores(
	...
	if err := pc.Retrieve(
		...
		if fault.Is(err, &vimtypes.ManagedObjectNotFound{}) {
			return nil, errors.New("invalid datastore ID")
		}
...
func newContentLibraryProviderOrDefault(
	...
	if fn, ok := obj.(func(context.Context, *rest.Client) clprov.Provider); ok {
...
func newCacheStorageURIsClientOrDefault(
	...
	if fn, ok := obj.(func(*vim25.Client) clsutil.CacheStorageURIsClient); ok {
