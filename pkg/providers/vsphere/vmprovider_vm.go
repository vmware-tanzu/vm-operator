...
func (vs *vSphereVMProvider) getVirtualMachineSnapshotsForVM(
	...
	for _, snapshot := range snapshotList.Items {
		if snapshot.Spec.VMRef != nil && snapshot.Spec.VMRef.Name == vmCtx.VM.Name {
			// Skip the snapshots that are not owned by the current
			// VM. It is possible that the snapshot controller is
			// still adding the owner reference.
			owner, err := controllerutil.HasOwnerReference(snapshot.OwnerReferences, vmCtx.VM, vs.k8sClient.Scheme())
			if err != nil {
			...
			if !owner {
				return nil, fmt.Errorf("snapshot is not owned by the VM resource")
			}
