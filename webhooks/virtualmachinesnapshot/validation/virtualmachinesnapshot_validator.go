...
// validateVMControllerSharingMode checks if the referenced VM has controller with sharingMode that's not None.
func (v validator) validateVMControllerSharingMode(
	...
	// Check if the VM has controllers with Physical sharing mode.
	if vm.Spec.Hardware != nil {
		for _, c := range vm.Spec.Hardware.SCSIControllers {
			if c.SharingMode != vmopv1.VirtualControllerSharingModeNone {
				allErrs = append(allErrs,
					field.Forbidden(vmNameField,
						fmt.Sprintf(errUnsupportedVMControllerSharingModeFmt, "SCSIControllers", c.BusNumber, c.SharingMode)),
				)
			}
		}
	}
...
func (v validator) validateVMFields(
	...
	if err := v.client.Get(ctx, vmKey, &vm); err != nil {
		if apierrors.IsNotFound(err) {
			...
			ctx.Logger.V(4).Info("Unable to get VM for controller sharingMode validation, allowing snapshot", "vmName", vmName, "error", err)
			return nil
		}
