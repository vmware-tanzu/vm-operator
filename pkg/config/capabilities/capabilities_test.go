		...
		When("the resource exists", func() {
			...
			Context("with true capabilities", func() {
				BeforeEach(func() {
					...
					obj.Status.Supervisor = map[capv1.CapabilityName]capv1.CapabilityStatus{
						...
						capabilities.CapabilityKeyVMEvacuation: {
							Activated: true,
						},
					}
				...
				When("the capabilities are not different", func() {
					...
					Specify(capabilities.CapabilityKeyVMEvacuation, func() {
						Expect(pkgcfg.FromContext(ctx).Features.VMEvacuation).To(BeTrue())
					})
				})
			...
			Context("with false capabilities", func() {
				...
				When("the capabilities are not different", func() {
					...
					Specify(capabilities.CapabilityKeyVMEvacuation, func() {
						Expect(pkgcfg.FromContext(ctx).Features.VMEvacuation).To(BeFalse())
					})
				})
...
var _ = Describe("UpdateCapabilitiesFeatures", func() {
	...
	When("obj is capv1.Capabilities", func() {
		...
		Context(capabilities.CapabilityKeyVSpherePolicies, func() {
			BeforeEach(func() {
				Expect(pkgcfg.FromContext(ctx).Features.VSpherePolicies).To(BeFalse())
				obj.Status.Supervisor[capabilities.CapabilityKeyVSpherePolicies] = capv1.CapabilityStatus{
					Activated: true,
				}
			})
			Specify("Enabled", func() {
				Expect(ok).To(BeTrue())
				Expect(diff).To(Equal("VSpherePolicies=true"))
				Expect(pkgcfg.FromContext(ctx).Features.VSpherePolicies).To(BeTrue())
			})
		})
		...
		Context(capabilities.CapabilityKeyStoragePolicyMutability, func() {
			BeforeEach(func() {
				Expect(pkgcfg.FromContext(ctx).Features.StoragePolicyMutability).To(BeFalse())
				obj.Status.Supervisor[capabilities.CapabilityKeyStoragePolicyMutability] = capv1.CapabilityStatus{
					Activated: true,
				}
			})
			Specify("Enabled", func() {
				Expect(ok).To(BeTrue())
				Expect(diff).To(Equal("StoragePolicyMutability=true"))
				Expect(pkgcfg.FromContext(ctx).Features.StoragePolicyMutability).To(BeTrue())
			})
		})
		...
		Context(capabilities.CapabilityKeyPerNamespaceNetworkProvider, func() {
			BeforeEach(func() {
				Expect(pkgcfg.FromContext(ctx).Features.PerNamespaceNetworkProvider).To(BeFalse())
				obj.Status.Supervisor[capabilities.CapabilityKeyPerNamespaceNetworkProvider] = capv1.CapabilityStatus{
					Activated: true,
				}
			})
			Specify("Enabled", func() {
				Expect(ok).To(BeTrue())
				Expect(diff).To(Equal("PerNamespaceNetworkProvider=true"))
				Expect(pkgcfg.FromContext(ctx).Features.PerNamespaceNetworkProvider).To(BeTrue())
			})
		})
		...
		Context(capabilities.CapabilityKeyWorkloadNetworkConfiguration, func() {
			BeforeEach(func() {
				Expect(pkgcfg.FromContext(ctx).Features.WorkloadNetworkConfiguration).To(BeFalse())
				obj.Status.Supervisor[capabilities.CapabilityKeyWorkloadNetworkConfiguration] = capv1.CapabilityStatus{
					Activated: true,
				}
			})
			Specify("Enabled", func() {
				Expect(ok).To(BeTrue())
				Expect(diff).To(Equal("WorkloadNetworkConfiguration=true"))
				Expect(pkgcfg.FromContext(ctx).Features.WorkloadNetworkConfiguration).To(BeTrue())
			})
		})
		...
		Context(capabilities.CapabilityKeyVMEvacuation, func() {
			BeforeEach(func() {
				Expect(pkgcfg.FromContext(ctx).Features.VMEvacuation).To(BeFalse())
				obj.Status.Supervisor[capabilities.CapabilityKeyVMEvacuation] = capv1.CapabilityStatus{
					Activated: true,
				}
			})
			Specify("Enabled", func() {
				Expect(ok).To(BeTrue())
				Expect(diff).To(Equal("VMEvacuation=true"))
				Expect(pkgcfg.FromContext(ctx).Features.VMEvacuation).To(BeTrue())
			})
		})
	})
