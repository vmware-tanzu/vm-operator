...
func (r *Reconciler) ReconcileNormal(ctx *pkgctx.VolumeContext) error {
	...
	if pkgcfg.FromContext(ctx).Features.InstanceStorage &&
		...
		if r.GetInstanceStoragePVCClient == nil {
			return fmt.Errorf("GetInstanceStoragePVCClient is not initialized")
		}
...
func isSnapshotVolume(vm *vmopv1.VirtualMachine, volName string) bool {
	return vmopv1util.IsSnapshotVolume(vm, volName)
}
...
func shouldDeleteVolumeStatus(vm *vmopv1.VirtualMachine, e vmopv1.VirtualMachineVolumeStatus) bool {
	return vmopv1util.ShouldDeleteVolumeStatus(vm, e)
}
