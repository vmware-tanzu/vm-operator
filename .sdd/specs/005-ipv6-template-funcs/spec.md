...
- ~~E2E dual-stack coverage — does the E2E harness used in CI support provisioning a VM with a genuine dual-stack (IPv4 + IPv6) NIC under the `WorkloadIPv6` capability today?~~ **Resolved: no**, for a *guaranteed* dual-stack NIC — no IP-family knob in `test/e2e/manifestbuilders`, no IPv6/dual-stack CIDR fixture under `test/e2e/fixtures/yaml/vmoperator/subnet/`, and the standard CI testbed (`test/e2e/README.md`'s `vds_standard_medium`) is IPv4-only. That part remains a tracked infrastructure follow-up. However, a real (not infra-blocked) E2E scenario was still added: reusing the `photon-ovf-vapp-properties` OVF image's already-defined, already-tested vApp properties (from the existing "Multiple vAppConfigs specified" test) and setting their values to V1alpha6 template-function calls sidesteps the separate "unknown property key gets silently dropped" problem entirely — one property per function under test. `V1alpha6_FirstIPv4`/`FirstIPv4FromNIC` are asserted strictly (IPv4 is guaranteed via `LinuxPrep`); `FirstIPv6`/`FirstIPv6FromNIC` are asserted leniently (valid IPv6 CIDR, or gracefully-unrendered — this test doesn't control whether its subnet hands out IPv6). See `plan.md` Test strategy for the full design and the still-open multi-NIC/genuine-dual-stack follow-ups.
