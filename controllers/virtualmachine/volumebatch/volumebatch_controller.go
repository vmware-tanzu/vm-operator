...
func (r *Reconciler) ReconcileNormal(ctx *pkgctx.VolumeContext) error {
	...
	// Get existing VM managed volumes status
	...
	volumeSpecForBatch, volumeSpecForLegacy := categorizeVolumeSpecs(ctx, legacyAttachments)
	...
	if batchAttachment == nil && len(volumeSpecForBatch) > 0 {
	...
	volumeStatusesFromBatchAttachment, processErr := r.processBatchAttachment(
		ctx,
		batchAttachment,
	...
	volumeStatusesFromLegacyAttachments := r.processLegacyAttachments(
		ctx,
		legacyAttachments,
		attachmentsToDelete,
		existingVMManagedVolStatus,
	...
	updateVMVolumeStatus(
		ctx,
		volumeStatusesFromBatchAttachment,
		volumeStatusesFromLegacyAttachments,
	)
...
// processBatchAttachment processes the batch attachment and returns the
// volume statuses that are tracked by the batch attachment.
func (r *Reconciler) processBatchAttachment(
...
) ([]vmopv1.VirtualMachineVolumeStatus, error) {
	...
	if len(vmVolumeSpecsForBatch) == 0 {
		// The batch attachment is not needed.
		// Still need to continue to clean up the volume statuses.
		if existingAttachment != nil {
			...
			if err != nil && !apierrors.IsNotFound(err) {
				retErr = errOrNoRequeueErr(retErr,
					fmt.Errorf("failed to delete CnsNodeVMBatchAttachment: %w", err),
				)
			}
		}
	}
	...
	var (
		// Record the volume specs to be updated in vm.status.volumes.
	...
	volumeStatuses := r.getVMVolStatusFromBatchAttachment(
		ctx,
		existingAttachment,
		volumeSpecs,
		existingVMManagedVolStatus,
...
// getVMVolStatusFromBatchAttachment filters vm managed volumes status with
...
func (r *Reconciler) getVMVolStatusFromBatchAttachment(
	ctx *pkgctx.VolumeContext,
	existingAttachment *cnsv1alpha1.CnsNodeVMBatchAttachment,
	volumeSpecs []cnsv1alpha1.VolumeSpec,
	existingVMManagedVolStatus map[string]vmopv1.VirtualMachineVolumeStatus,
	...
	getVolumeStatusWithDetachingVolumeFromBatchAttachment(
		existingAttachVolStatus, &volumeStatuses,
	)
...
// getVolumeStatusWithDetachingVolumeInLegacyAttachment checks legacy
// orphaned attachments and find volumes that are being detached, return them.
func getVolumeStatusWithDetachingVolumeInLegacyAttachment(
	ctx *pkgctx.VolumeContext,
...
// in detaching state in batchAttachment.status to the volumeStatuses.
...
// processLegacyAttachments processes the legacy attachments and returns the
// volume statuses that are tracked by the legacy attachments.
func (r *Reconciler) processLegacyAttachments(
	ctx *pkgctx.VolumeContext,
	...
	for _, vol := range vmVolumeSpecsForLegacy {
		...
		// with CSI.
		// If this volume has a legacyAttachment associated.
		...
		if att, ok := legacyAttachments[attachmentNameForVol]; ok {
			...
			if vol.PersistentVolumeClaim.ClaimName == att.Spec.VolumeName {
				...
				// If PVC has been changed for this legacy volume, remove it from
				// orphanedAttachments so that later this volume won't be added
				// to vm volume status as a detaching volume. Since its status might
				// still be Attached, which will be confusing for the user.
				// But with this logic there might be a window when the ClaimName
				// is changed but before CNS reconciles the volume for detaching,
				// then user could get a stale data in this case.
				// Changing the ClaimName should be a rare case.
				for i := range orphanedAttachments {
					if orphanedAttachments[i].Name == att.Name {
	...
	getVolumeStatusWithDetachingVolumeInLegacyAttachment(
		ctx,
		orphanedAttachments,
		&volumeStatuses,
	)
...
// updateVMVolumeStatus adds clears any managed volume status then adds given
...
func categorizeVolumeSpecs(
	ctx *pkgctx.VolumeContext,
	legacyAttachments map[string]cnsv1alpha1.CnsNodeVmAttachment,
) (forBatch []vmopv1.VirtualMachineVolume, forLegacy []vmopv1.VirtualMachineVolume) {
	...
	for _, vol := range ctx.VM.Spec.Volumes {
		...
		if legacyAttachment, ok := legacyAttachments[attachmentNameForVol]; ok {
			...
			// name changed, since need to remove it from VM volume status.
