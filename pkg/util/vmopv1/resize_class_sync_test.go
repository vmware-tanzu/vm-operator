...
var _ = Describe("SyncClassComputeToSpec", func() {
	...
	Context("classCS NumCPUs not evenly divisible by VirtualNuma.CoresPerNumaNode", func() {
		// 10 CPUs / 4 cores-per-node = 2.5, truncates to 2. Deriving
		// VNUMANodeCount=2 here would round-trip through
		// vnumaNodeCountField's inverse division in compute_overwrite.go
		// (CoresPerNumaNode = 10/2 = 5) and silently drift from the
		// class's actual CoresPerNumaNode=4.
		BeforeEach(func() {
			classCS.NumCPUs = 10
			classCS.VirtualNuma = &vimtypes.VirtualMachineVirtualNuma{
				CoresPerNumaNode: ptr.To(int32(4)),
