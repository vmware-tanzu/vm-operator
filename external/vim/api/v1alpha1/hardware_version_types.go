...
// vmxRe is used by ParseHardwareVersion below. This is up here to ensure it
// stays in lock-step with the pattern used in the below kubebuilder statement.
var vmxRe = regexp.MustCompile(`(?i)^(?:vmx-)?(\d+)$`)

// +kubebuilder:validation:Type=string
// +kubebuilder:validation:Pattern=`(?i)^(?:vmx-)?(\d+)$`
...
func (hv HardwareVersion) MarshalJSON() ([]byte, error) {
	return json.Marshal(hv.String())
}

func (hv *HardwareVersion) UnmarshalJSON(data []byte) error {
	var s string
	if err := json.Unmarshal(data, &s); err != nil {
		return err
	}
	return hv.UnmarshalText([]byte(s))
}
