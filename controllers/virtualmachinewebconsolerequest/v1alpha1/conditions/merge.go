...
func merge(conditions []localizedCondition, targetCondition vmopv1a1.ConditionType, options *mergeOptions) *vmopv1a1.Condition {
...
func (g conditionGroups) getByStatusAndSeverity(status corev1.ConditionStatus, severity vmopv1a1.ConditionSeverity) *conditionGroup {
	if len(g) == 0 {
		return nil
	}
	for _, group := range g {
