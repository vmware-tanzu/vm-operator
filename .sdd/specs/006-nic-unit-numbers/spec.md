...
- **No SR-IOV passthrough placement.** Physical/virtual function placement is out of scope. SR-IOV NICs in `spec.network.interfaces` are still ethernet cards on the virtual PCI bus and *are* covered by this feature's placement, validation, backfill, and matching like any other interface type (confirmed by **Q3**).
...
- **Removing a device at a slot and adding a device at that same slot within one reconfigure is supported platform behaviour**, and is what lets **G12**'s replacement resolve in a single reconcile with no intermediate state. The research program should confirm it once on the target vCenter build; the simulator is not authoritative for it, since its duplicate-unit-number check is scoped to devices sharing a controller key and device type.
...
### Gating — implementation is blocked until these are answered

- **Q1** — [NEEDS CLARIFICATION] *(blocks the add and create paths; blocks whether the design converges at all)* Does vSphere honour an explicit unit number on an add operation for a NIC, or does it place the device wherever it likes? Under **G11** there is no edit-based correction step: if an add lands at a different slot than requested, the next reconcile's exact-slot lookup misses again and adds again, removing the previous attempt each time — churn that never settles rather than a one-time cosmetic miss. A "not honoured" answer means spec-driven placement cannot ship as designed and this spec must be revisited.
- **Q2** — [NEEDS CLARIFICATION] *(may require per-path gating of the create path)* Does the OVF content-library deploy path honour an explicit unit number carried on a NIC device in the deployment's config spec, and how does it interact with NICs already present in the OVF descriptor? This is a distinct code path from the direct create-VM call — not the dominant one, since fast deploy is preferred when enabled, but reachable whenever fast deploy is off. A different answer from **Q1**'s means the create path needs a per-path gate rather than one blanket check.
- **Q3** — [NEEDS CLARIFICATION] *(scopes the SR-IOV claim in Non-goals)* Do SR-IOV ethernet cards occupy the same unit-number space as other NIC types? This spec asserts they are covered by the same placement, validation, backfill, and matching. **Environment prerequisite:** the experiment needs an SR-IOV-capable host and network, which not every test environment has; an unavailable environment must be recorded as an explicit skip with a reason, not silently omitted.
- **Q4** — [NEEDS CLARIFICATION] *(drives error handling on a failed add and a failed create)* What fault does vSphere return when an explicit unit number collides with an existing device? A collision is reachable despite uniqueness validation — against a VM class ConfigSpec device, or a NIC added out of band through the vCenter UI. The answer determines whether the reconciler treats the failure as permanent or retries.

### To characterize — recorded, not blocking

These have consumers in `tasks.md` (research program, E2E, or integration tests) and their answers are documented rather than gating.

- **Q5** — [NEEDS CLARIFICATION] When a NIC is removed and a new one added, does vSphere reuse the freed slot or take the next available one?
- **Q6** — [NEEDS CLARIFICATION] Are NIC unit numbers stable across VM power cycles?
- **Q7** — [NEEDS CLARIFICATION] What happens to unit numbers when a NIC is added out of band through the vCenter UI? Existing behaviour for un-numbered interfaces already shifts under such an add; this feature makes no design change for it, and the answer is recorded so operators know what to expect.
- **Q8** — [NEEDS CLARIFICATION] Snapshot revert interplay: the snapshot's backup data restores the VM spec and annotations, so unit numbers and the feature-version bit travel with the snapshot, while the imported-VM fallback synthesizes interfaces without unit numbers. Reverting to a pre-backfill snapshot drops the bit and re-triggers the backfill — confirm it converges.
