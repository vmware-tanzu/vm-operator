...
func unitTestsValidateCreate() {
	...
	validateCreate := func(args createArgs, expectedAllowed bool, expectedReason string) {
		vmco := builder.DummyVirtualMachineConfigOptions("test-vmco", args.hardwareVersion)
		obj, err := builder.ToUnstructured(vmco)
		Expect(err).ToNot(HaveOccurred())
	...
	When("the hardwareVersion is valid", func() {
		It("should allow creation with vmx-N format", func() {
			validateCreate(createArgs{hardwareVersion: "vmx-21"}, true, "")
		})

		It("should allow creation with a large version number", func() {
			validateCreate(createArgs{hardwareVersion: "vmx-1000"}, true, "")
		})

		It("should allow creation with version 0", func() {
			validateCreate(createArgs{hardwareVersion: "vmx-0"}, true, "")
		})
	})

	When("the hardwareVersion is empty", func() {
		It("should deny creation", func() {
			validateCreate(createArgs{hardwareVersion: ""}, false, "hardwareVersion")
		})
	})
	...
	When("the hardwareVersion does not match the required pattern", func() {
		It("should deny creation when the numeric suffix is missing", func() {
			validateCreate(createArgs{hardwareVersion: "vmx-"}, false, "hardwareVersion")
		})

		It("should deny creation without the vmx- prefix", func() {
			validateCreate(createArgs{hardwareVersion: "21"}, false, "hardwareVersion")
		})

		It("should deny creation with a non-numeric suffix", func() {
			validateCreate(createArgs{hardwareVersion: "vmx-abc"}, false, "hardwareVersion")
		})

		It("should deny creation with extra characters after the digits", func() {
			validateCreate(createArgs{hardwareVersion: "vmx-21a"}, false, "hardwareVersion")
		})

		It("should deny creation with an uppercase prefix", func() {
			validateCreate(createArgs{hardwareVersion: "VMX-21"}, false, "hardwareVersion")
		})
	})
}
