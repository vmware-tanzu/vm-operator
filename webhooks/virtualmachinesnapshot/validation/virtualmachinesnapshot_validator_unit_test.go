...
func unitTestsValidateCreate() {
	...
	DescribeTable("validate VM", validateCreate,
		...
		Entry("should deny snapshot if controller has physical sharing mode",
			...
			false,
			field.Forbidden(vmNameField, "controller type SCSIControllers bus 0 is using unsupported sharingMode for snapshot: Physical").Error(),
			nil,
		),
		Entry("should deny snapshot if controller has virtual sharing mode",
			...
			false,
			field.Forbidden(vmNameField, "controller type SCSIControllers bus 0 is using unsupported sharingMode for snapshot: Virtual").Error(),
			nil,
		),
