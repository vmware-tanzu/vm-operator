...
var _ = Describe(
	...
	func() {
		...
		assertLocation := func(
			...
			g.ExpectWithOffset(1, status.Files).To(HaveLen(1))
			g.ExpectWithOffset(1, status.Files[0]).To(Equal(vmdkFilePath))
		}
		...
		Context("Ordered", Ordered, func() {
			...
			DescribeTable("Failures",
				...
				Entry(
					...
					true, "", // OVFReady
					true, "", // ProviderReady
					false, "invalid datacenter ID", "", // DisksReady
				...
				Entry(
					...
					true, "", // OVFReady
					true, "", // ProviderReady
					false, "invalid datastore ID", "", // DisksReady
					false, "2 of 3 completed", // Ready
				),
