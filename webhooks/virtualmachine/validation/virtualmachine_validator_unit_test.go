...
func unitTestsValidateCreate() {
		...
		DescribeTable("create", doTest,
			...
			Entry("should deny Snapshot volume referencing non-existent VirtualMachineSnapshot",
				testParams{
					...
					expectAllowed: false,
					validate: doValidateWithMsg(
						field.NotFound(volPath.Index(0).Child("virtualMachineSnapshot", "name"), "non-existent-snap").Error(),
					),
				},
			),
			Entry("should deny Snapshot volume referencing VirtualMachineSnapshot that is not ready",
				testParams{
					...
					expectAllowed: false,
					validate: doValidateWithMsg(
						field.Invalid(volPath.Index(0).Child("virtualMachineSnapshot", "name"), "unready-snap", "VirtualMachineSnapshot unready-snap is not ready").Error(),
					),
				},
			),
			Entry("should deny Snapshot volume referencing diskID that does not appear in status.disks",
				testParams{
					...
					expectAllowed: false,
					validate: doValidateWithMsg(
						field.Invalid(volPath.Index(0).Child("virtualMachineSnapshot", "diskID"), "missing-disk-id", "diskID missing-disk-id does not appear in status.disks of VirtualMachineSnapshot snap-with-other-disk").Error(),
					),
				},
			),
		)
	...
	Context("Readiness Probe", func() {
		...
		DescribeTable("create", doTest,
			...
			Entry("should deny TCP readiness probe with VPC via PerNamespaceNetworkProvider",
				testParams{
					setup: func(ctx *unitValidatingWebhookContext) {
						ctx.vm.Spec.ReadinessProbe = &vmopv1.VirtualMachineReadinessProbeSpec{
							TCPSocket: &vmopv1.TCPSocketAction{},
		...
		DescribeTable("VolumeAttributesClass create", doTest,
			...
			Entry("volume attributes class not specified and storage class missing policyID",
				testParams{
					...
					validate: doValidateWithMsg(
						`spec.storageClass: Invalid value: "dummy-storage-class": StorageClass "dummy-storage-class" does not have "storagePolicyID" parameter`),
				},
			),
	...
	Context("Network", func() {
		...
		DescribeTableSubtree("network create - validate network provider API group and kind",
			func(
				...
				var (
					capProviderType netopv1alpha1.NetworkProvider
					apiVersion      string
					kind            string

					wrongAPIVersion    string
					wrongKind          string
					wrongAPIVersionMsg string
					wrongKindMsg       string
				)
				...
				BeforeEach(func() {
					...
					case pkgcfg.NetworkProviderTypeVDS:
						capProviderType = netopv1alpha1.NetworkProviderVSphereDistributed
						apiVersion = "netoperator.vmware.com/v1alpha1"
						kind = "Network"

						wrongAPIVersion = "vmware.com/v1alpha1"
						wrongKind = "VirtualNetwork"
						wrongAPIVersionMsg = `spec.network.interfaces[0].network.apiVersion: Unsupported value: "vmware.com/v1alpha1": supported values: "netoperator.vmware.com/v1alpha1"`
						wrongKindMsg = `spec.network.interfaces[1].network.kind: Unsupported value: "VirtualNetwork": supported values: "Network"`
					case pkgcfg.NetworkProviderTypeNSXT:
						capProviderType = netopv1alpha1.NetworkProviderNSXTier1
						apiVersion = "vmware.com/v1alpha1"
						kind = "VirtualNetwork"

						wrongAPIVersion = "crd.nsx.vmware.com/v1alpha1"
						wrongKind = "SubnetSet"
						wrongAPIVersionMsg = `spec.network.interfaces[0].network.apiVersion: Unsupported value: "crd.nsx.vmware.com/v1alpha1": supported values: "vmware.com/v1alpha1"`
						wrongKindMsg = `spec.network.interfaces[1].network.kind: Unsupported value: "SubnetSet": supported values: "VirtualNetwork"`
					case pkgcfg.NetworkProviderTypeVPC:
						capProviderType = netopv1alpha1.NetworkProviderVPC
						apiVersion = "crd.nsx.vmware.com/v1alpha1"
						kind = "SubnetSet"
					...
					if enableCapability {
						Expect(ctx.Client.Create(ctx, &netopv1alpha1.NetworkSettings{
							...
							Provider: capProviderType,
						})).To(Succeed())
					}
				})

				It("allows correct APIVersion and Kind", func() {
					ctx.vm.Spec.Network = &vmopv1.VirtualMachineNetworkSpec{
						Interfaces: []vmopv1.VirtualMachineNetworkInterfaceSpec{
							{
								Name: "eth0",
				...
				It("denies incorrect API group and kind", func() {
					ctx.vm.Spec.Network = &vmopv1.VirtualMachineNetworkSpec{
						Interfaces: []vmopv1.VirtualMachineNetworkInterfaceSpec{
							{
								Name: "eth0",
					...
					doValidateWithMsg(wrongAPIVersionMsg, wrongKindMsg)(response)
				})
			},

			Entry("nsx-tier1 envvar", pkgcfg.NetworkProviderTypeNSXT, false),
			Entry("nsx-tier1 capability", pkgcfg.NetworkProviderTypeNSXT, true),
			Entry("vsphere-distributed envvar", pkgcfg.NetworkProviderTypeVDS, false),
			Entry("vsphere-distributed capability", pkgcfg.NetworkProviderTypeVDS, true),
			Entry("vpc envvar", pkgcfg.NetworkProviderTypeVPC, false),
		...
		Context("when PerNamespaceNetworkProvider is enabled and NetworkSettings is absent", func() {
			...
			It("ValidateCreate returns error", func() {
				var err error
				ctx.WebhookRequestContext.Obj, err = builder.ToUnstructured(ctx.vm)
				Expect(err).ToNot(HaveOccurred())
				response := ctx.ValidateCreate(&ctx.WebhookRequestContext)
				Expect(response.Allowed).To(BeFalse())
				Expect(response.Result.Message).To(ContainSubstring(netsetutil.ErrNetworkSettingsNotFound.Error()))
			})
		})
...
func unitTestsValidateUpdate() { //nolint:gocyclo
	...
	Context("Backfilled Volume validation", func() {
		DescribeTable("Updates", doTest,
			Entry("should deny removing a backfilled volume",
				testParams{
					setup: func(ctx *unitValidatingWebhookContext) {
						// Create a backfilled volume (no PVC, but has target ID)
						...
						// Remove it from new VM
						ctx.vm.Spec.Volumes = slices.DeleteFunc(ctx.vm.Spec.Volumes, func(v vmopv1.VirtualMachineVolume) bool {
							return v.Name == "backfilled-vol"
						})
					},
					expectAllowed: false,
			...
			Entry("should allow when backfilled volume is unchanged",
				testParams{
					setup: func(ctx *unitValidatingWebhookContext) {
						...
						// Keep it unchanged in new VM
						ctx.vm.Spec.Volumes = append(ctx.vm.Spec.Volumes, *backfilledVol.DeepCopy())
					},
					expectAllowed: true,
				},
			),
			Entry("should allow removing a non-backfilled volume (has PVC)",
				testParams{
					setup: func(ctx *unitValidatingWebhookContext) {
						// Create a volume with PVC (not backfilled)
						pvcVol := vmopv1.VirtualMachineVolume{
							Name: "pvc-vol",
							VirtualMachineVolumeSource: vmopv1.VirtualMachineVolumeSource{
								PersistentVolumeClaim: &vmopv1.PersistentVolumeClaimVolumeSource{
									PersistentVolumeClaimVolumeSource: corev1.PersistentVolumeClaimVolumeSource{
						...
						ctx.oldVM.Spec.Volumes = append(ctx.oldVM.Spec.Volumes, pvcVol)
						// Remove it from new VM
						ctx.vm.Spec.Volumes = slices.DeleteFunc(ctx.vm.Spec.Volumes, func(v vmopv1.VirtualMachineVolume) bool {
							return v.Name == "pvc-vol"
						})
					},
					expectAllowed: true,
				},
			),
			Entry("should allow when volume has no target ID (not backfilled)",
				testParams{
					setup: func(ctx *unitValidatingWebhookContext) {
						// Create a volume without target ID (missing placement fields)
						volWithoutTargetID := vmopv1.VirtualMachineVolume{
							Name: "vol-no-target",
							// No PersistentVolumeClaim
							// No ControllerType, ControllerBusNumber, or UnitNumber
						}
						ctx.oldVM.Spec.Volumes = append(ctx.oldVM.Spec.Volumes, volWithoutTargetID)
						// Remove it from new VM
						ctx.vm.Spec.Volumes = slices.DeleteFunc(ctx.vm.Spec.Volumes, func(v vmopv1.VirtualMachineVolume) bool {
							return v.Name == "vol-no-target"
						})
					},
					expectAllowed: true,
				},
