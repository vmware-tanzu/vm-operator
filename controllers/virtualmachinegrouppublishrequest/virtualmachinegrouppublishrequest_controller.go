...
func AddToManager(ctx *pkgctx.ControllerManagerContext, mgr manager.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&vmopv1.VirtualMachinePublishRequest{},
			handler.EnqueueRequestForOwner(mgr.GetScheme(), mgr.GetRESTMapper(), &vmopv1.VirtualMachineGroupPublishRequest{})).
		Complete(r)
}
...
func (r *Reconciler) reconcileStatusImages(
	...
	sort.Slice(imageStatuses, func(i, j int) bool {
		return imageStatuses[i].Source < imageStatuses[j].Source
	})

	ctx.VMGroupPublishRequest.Status.Images = imageStatuses
}
		...
		if err := r.Create(ctx, &vmopv1.VirtualMachinePublishRequest{
			...
			Spec: vmopv1.VirtualMachinePublishRequestSpec{
				Source: vmopv1.VirtualMachinePublishRequestSource{
					Name:       sourceVM,
					Kind:       reflect.TypeOf(vmopv1.VirtualMachine{}).Name(),
				...
				Target: vmopv1.VirtualMachinePublishRequestTarget{
					Location: vmopv1.VirtualMachinePublishRequestTargetLocation{
						...
						Kind:       reflect.TypeOf(imgregv1a1.ContentLibrary{}).Name(),
...
func (r *Reconciler) reconcileSpecTTL(ctx *pkgctx.VirtualMachineGroupPublishRequestContext) error {
	...
	ctx.Logger.Info("deleting vm group publish request")
	if err := r.Delete(ctx, vmGroupPubReq); err != nil {
		ctx.Logger.Error(err, "failed to delete vm publish request")
		return err
	}
	ctx.Logger.Info("deleted vm group publish request")
	return nil
}
