...
func GetLinuxPrepSecretData(
	...
	if pw := linuxPrep.Password; pw != nil {
		key := ctrlclient.ObjectKey{Name: pw.Password.Name, Namespace: secretNamespace}
		secret := &corev1.Secret{}
		if err := k8sClient.Get(ctx, key, secret); err != nil {
			return SecretData{}, fmt.Errorf("failed to get secret for password: %w", err)
		}

		password, ok := secret.Data[pw.Password.Key]
		if !ok {
			err := fmt.Errorf("secret %s does not contain required password key %q", key.Name, pw.Password.Key)
