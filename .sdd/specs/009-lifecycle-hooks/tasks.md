...
design (not a blocker for Phase 1/2 — see plan.md "Blocking items before
...
- [ ] T008 Implement `pkg/lifecycle/stage.go` — `ReconcileStage(ctx, k8sClient, obj, stageName) (Result, error)` implementing the get-or-create / pause / resume decision table from `model.md`, mirroring `HooksReady` as the sole non-`True`/`True` signal (no failure-detail parsing).
- [ ] T009 [P] Unit tests for `ReconcileStage`'s full decision table in `pkg/lifecycle/stage_test.go`
...
- [ ] T013 [US1] Wire `pkg/lifecycle.ReconcileStage` for the `Create` stage into `controllers/virtualmachine/virtualmachine/virtualmachine_controller.go`'s `ReconcileNormal`, before the provider's create path
...
- [ ] T024 [US4] Unit tests confirming `Features.LifecycleHooks=false` makes every `ReconcileStage` call in T013/T017/T020/T021 a pure no-op (no `LifecycleState` `Get`/`Create`, no pause)
