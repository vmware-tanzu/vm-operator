...
- **Status**: In Progress
- **Epic**: vmop-4013
- **Spike (resolved)**: vmop-4030 — the VBR REST API is reachable on port `9419` (VBR 13.1, API `1.3-rev2`); see [`research.md`](./research.md).
...
- The pipeline MUST accept the target Veeam server's address as a parameter, defaulting to the pre-existing Veeam appliance used for manual testing today when the parameter is not supplied, so a run against a different Veeam instance does not require a code change. The defaults live in the internal CI suite definition, not in this public repository, which only reads `VEEAM_*` environment variables.
- The pipeline MUST accept Veeam server credentials as a parameter, defaulting to the pre-existing appliance's known credentials when not supplied. The defaults live in the internal CI suite definition, not in this public repository, which only reads `VEEAM_*` environment variables.
...
- Tests MUST skip (not fail) with a clear message when the configured Veeam server is unreachable or does not expose a REST API version the tooling knows how to speak, mirroring the suite's existing skip-on-missing-capability conventions. **Amended:** only a missing server configuration skips. A configured server that is unreachable or speaks no supported API version fails the test, so a misconfigured CI suite is debugged instead of passing silently; see [`plan.md`](./plan.md) "Skip vs. fail."
...
- ~~Is the VBR REST API reachable?~~ **Resolved** (vmop-4030): the appliance runs VBR 13.1 and serves REST API `1.3-rev2` on port `9419`. Clients must bypass the E2E environment's HTTP(S) proxy to reach it.
- ~~Is a Veeam server already provisioned...~~ **Resolved**: a VBR appliance already runs on a Windows Server VM and is used manually today. The pipeline accepts its address as a parameter.
- ~~Which VBR release and REST API version should be targeted...~~ **Resolved**: the version is detected at connect time from the per-version Swagger documents; the client supports `1.1-rev0` through `1.3-rev2`.
- ~~How are Veeam service-account credentials distributed...~~ **Resolved**: credentials are pipeline parameters (`VEEAM_USERNAME` / `VEEAM_PASSWORD`). No Kubernetes Secret plumbing is required.
- ~~Does "restore to new" mean Veeam's "restore VM" flow or an "instant recovery" flow?~~ **Resolved**: Veeam's standard entire-VM restore, targeting the original location after the VM is lost.
- ~~Should disk-only restore use Veeam's "virtual disk restore" or an FCD instant-recovery-and-migrate sequence?~~ **Resolved (re-scoped)**: on `1.3-rev2` a restore point's `allowedOperations` include no virtual-disk restore, and FCD instant recovery does not produce what a VADP disk restore produces. Disk-only restore stays simulated; see [`plan.md`](./plan.md) "Scope changes."
- ~~Does deleting a job remove its backups?~~ **Resolved**: backups are deleted explicitly with `DELETE /api/v1/backups/{id}?fromDB=false&includeGFS=true` before the job is deleted.
