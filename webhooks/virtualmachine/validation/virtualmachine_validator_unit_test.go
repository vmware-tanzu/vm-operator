		...
		DescribeTable("network create", doTest,
			...
			Entry("disallow creating a VM with network interface with a specified mac address for not support network providers",
				testParams{
					...
					validate: doValidateWithMsg(
						`spec.network.interfaces[0].macAddress: Invalid value: "00:00:00:00:BB:AA": macAddress is available only with the following network providers: crd.nsx.vmware.com`,
						`spec.network.interfaces[1].macAddress: Invalid value: "00:00:00:00:CC:DD": macAddress is available only with the following network providers: crd.nsx.vmware.com`,
					),
				},
			),
		)
			...
			DescribeTable("disallow updates", doTest,
				...
				Entry("disallow Network Interface MAC address change",
					testParams{
						...
						validate: doValidateWithMsg(`spec.network.interfaces[0].macAddress: Invalid value: "bar": field is immutable`),
					},
				),
			)
		})
							...
							ctx.vm.Spec.Network = &vmopv1.VirtualMachineNetworkSpec{
								Interfaces: []vmopv1.VirtualMachineNetworkInterfaceSpec{
									...
									},
									{
										...
										MacAddress: "my-new-mac",
									},
								},
							}
						},
						validate: doValidateWithMsg(
							`spec.network.interfaces[0].macAddress: Invalid value: "bar": field is immutable`,
							`spec.network.interfaces[2].macAddress: Invalid value: "my-new-mac": field is immutable`),
					},
				),
			)
		})
