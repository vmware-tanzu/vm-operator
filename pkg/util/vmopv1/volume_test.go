...
var _ = Describe("ConvertControllersGeneric", func() {

	Context("Empty controller map", func() {
		It("should return empty slice", func() {
			controllerMap := make(map[int32]*mockController)
			result := vmopv1util.ConvertControllersGeneric(controllerMap)
			Expect(result).To(BeEmpty())
		})
	})

	Context("Single controller", func() {
		It("should return controller with sorted devices", func() {
			controllerMap := map[int32]*mockController{
				1: {
					BusNumber: 1,
					Devices: []vmopv1.VirtualDeviceStatus{
						{Type: vmopv1.VirtualDeviceTypeDisk, UnitNumber: 2, Name: "disk2"},
						{Type: vmopv1.VirtualDeviceTypeCDROM, UnitNumber: 1, Name: "cdrom1"},
						{Type: vmopv1.VirtualDeviceTypeDisk, UnitNumber: 1, Name: "disk1"},
					},
				},
			}
			...
			Expect(result[0].GetBusNumber()).To(Equal(int32(1)))
			...
			Expect(devices[0].UnitNumber).To(Equal(int32(1)))
	...
	Context("Multiple controllers", func() {
		It("should return controllers sorted by bus number", func() {
			controllerMap := map[int32]*mockController{
				3: {
					BusNumber: 3,
					Devices: []vmopv1.VirtualDeviceStatus{
			...
			// Check that controllers are sorted by bus number
			Expect(result[0].GetBusNumber()).To(Equal(int32(1)))
			Expect(result[1].GetBusNumber()).To(Equal(int32(2)))
			Expect(result[2].GetBusNumber()).To(Equal(int32(3)))
		})
	})

	Context("Controllers with no devices", func() {
		It("should return controllers with empty device lists", func() {
			controllerMap := map[int32]*mockController{
				1: {BusNumber: 1, Devices: []vmopv1.VirtualDeviceStatus{}},
				2: {BusNumber: 2, Devices: nil},
			}

			result := vmopv1util.ConvertControllersGeneric(controllerMap)
			Expect(result).To(HaveLen(2))
			Expect(result[0].GetDevices()).To(BeEmpty())
			Expect(result[1].GetDevices()).To(BeEmpty())
		})
	})
