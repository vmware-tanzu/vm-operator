			...
			When("FSS WCP_VMService_FastDeploy is enabled", func() {
				...
				When("ovf is ready", func() {
					...
					When("files are ready", func() {
						...
						When("AllDisksArePVCs capability is enabled", func() {
							const (
								diskName   = "disk0"
								diskUUID   = "0c3684b0-0b17-4d0f-9f6c-7c8f829cb3f1"
								volumeName = "disk-from-image-volume"
							)

							addvolumeName := func(claimName string, claimType vmopv1.UnmanagedVolumeClaimVolumeType) {
								vm.Spec.Volumes = []vmopv1.VirtualMachineVolume{
									{
										Name: volumeName,
										VirtualMachineVolumeSource: vmopv1.VirtualMachineVolumeSource{
							...
							createPVC := func(name string, sc *string, size resource.Quantity) {
							...
							It("returns an error since PVC lacks storage class", func() {
								const claimName = "pvc-1"
								requestedSize := resource.MustParse("50Mi")
								createPVC(claimName, nil, requestedSize)

								addvolumeName(claimName, vmopv1.UnmanagedVolumeClaimVolumeTypeFromImage)

								_, err := createOrUpdateAndGetVcVM(ctx, vmProvider, vm)
								Expect(err).To(HaveOccurred())
								Expect(err.Error()).To(ContainSubstring(
									fmt.Sprintf(
										"pvc %s/%s has no storage class",
										vm.Namespace, claimName,
									),
								))
							})

							It("returns an error since PVC is non-existent", func() {
								const claimName = "pvc-1"
							...
							When("in Linked Clone mode", func() {
								parentDiskSize := resource.MustParse("30Mi")

								BeforeEach(func() {
									if vm.Annotations == nil {
										vm.Annotations = make(map[string]string)
									}
									vm.Annotations["vmoperator.vmware.com/fast-deploy"] = "Linked"
								})

								It("doesn't update capacity from PVC data source refs", func() {
									const claimName = "pvc-1"
									requestedSize := resource.MustParse("50Mi")
									createPVC(claimName, ptr.To(ctx.StorageClassName), requestedSize)
									...
									Expect(vcVM.Properties(ctx, vcVM.Reference(), []string{"config.hardware.device"}, &vmObj)).To(Succeed())

									devices := object.VirtualDeviceList(vmObj.Config.Hardware.Device)
									var matchedDisk *vimtypes.VirtualDisk
									for _, device := range devices {
										...
										if desc := disk.DeviceInfo.GetDescription(); desc != nil && desc.Label == diskName {
											matchedDisk = disk
											break
										}
									}

									Expect(matchedDisk).ToNot(BeNil())
									Expect(matchedDisk.Backing).ToNot(BeNil())
									Expect(matchedDisk.CapacityInBytes).ToNot(Equal(requestedSize.Value()))
									Expect(matchedDisk.CapacityInBytes).To(Equal(parentDiskSize.Value()))

									// TODO(nabarun): Once the govmomi PR https://github.com/vmware/govmomi/pull/3896
									// is merged and a new govmomi version is picked up, we can verify the UUID.
									// Expect(matchedDisk.Backing.(*vimtypes.VirtualDiskFlatVer2BackingInfo).Uuid).To(Equal(diskUUID))
								})
							})

							When("in Direct mode", func() {
								JustBeforeEach(func() {
									if vm.Annotations == nil {
										vm.Annotations = make(map[string]string)
									}
									vm.Annotations["vmoperator.vmware.com/fast-deploy"] = "Direct"
								})

								It("updates capacity from PVC data source refs", func() {
									const claimName = "pvc-1"
									requestedSize := resource.MustParse("50Mi")
									createPVC(claimName, ptr.To(ctx.StorageClassName), requestedSize)
									...
									Expect(matchedDisk).ToNot(BeNil())
									Expect(matchedDisk.Backing).ToNot(BeNil())
									Expect(matchedDisk.CapacityInBytes).To(Equal(requestedSize.Value()))
