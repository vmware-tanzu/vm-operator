...
import (
	...
	volumeproviders "github.com/vmware-tanzu/vm-operator/controllers/volume/providers"
...
func AddToManager(ctx *context.ControllerManagerContext, mgr manager.Manager) error {
	return add(ctx, mgr, newReconciler(mgr))
}
...
func (r *ReconcileVolume) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.reconcileNormal(ctx, vm); err != nil {
		r.log.Error(err, "Failed to reconcile VirtualMachine", "name", vm.NamespacedName())
		return reconcile.Result{}, err
	}
...
func (r *ReconcileVolume) reconcileNormal(ctx goctx.Context, origVM *vmoperatorv1alpha1.VirtualMachine) error {
	r.log.Info("Reconciling VirtualMachine for processing volumes ", "name", origVM.NamespacedName())
	defer func() {
...
func (r *ReconcileVolume) reconcileVolumes(ctx goctx.Context, vm *vmoperatorv1alpha1.VirtualMachine,
