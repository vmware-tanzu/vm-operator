...
func unitTestsReconcile() {
	...
	BeforeEach(func() {
		...
		vmSnapshot1.Status.Used = &size10GB
		vmSnapshot1.Status.Requested = []vmopv1.VirtualMachineSnapshotStorageStatusRequested{
			{
				StorageClass: inName,
				Total:        &size10GB,
			},
	...
	When("there is a StoragePolicyUsage resource for VMSnapshot", func() {
		...
		When("a VirtualMachineSnapshot is found", func() {
			...
			When("CSI driver has not completed the sync of the volume", func() {
				...
				When("VMSnapshot has empty status.requested", func() {
					BeforeEach(func() {
						vmSnapshot1.Status.Requested = nil
					})
				...
				When("The requested storage class doesn't match the SPU's storage class", func() {
					BeforeEach(func() {
						vmSnapshot1.Status.Requested[0].StorageClass = "different-storage-class"
					})
		...
		When("Two VirtualMachineSnapshots are found", func() {
			BeforeEach(func() {
				...
				vmSnapshot2.Status.Used = &size20GB
				vmSnapshot2.Status.Requested = []vmopv1.VirtualMachineSnapshotStorageStatusRequested{
					{
						StorageClass: inName,
						Total:        &size20GB,
					},
