...
**Garbage collection** (`controllers/configtarget/gc.go`): After the cluster-scope queries succeed:

```go
func GCVirtualMachineConfigOptions(ctx, client, clusterMoID, liveKeys []string) error
```

- Delete any `VirtualMachineConfigOptions` whose name is not in `liveKeys`.
- GC runs only when both `QueryConfigTarget` and `QueryConfigOptionDescriptor` succeed; per-host RPC outcomes do not gate it (per-host data lives inline on `ConfigTarget.status` and is overwritten each reconcile, so it does not produce stale K8s objects to delete).
