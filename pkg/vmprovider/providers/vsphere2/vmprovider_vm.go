...
func (vs *vSphereVMProvider) vmCreateGenConfigSpecExtraConfig(
	...
	if v, exists := ecMap[constants.ExtraConfigRunContainerKey]; exists {
		// For local-vcsim the JSON_EXTRA_CONFIG is set and contains RUN.container so we use the
		// template to fill in the image {{.Name }} so vcsim will create a container for the VM.
