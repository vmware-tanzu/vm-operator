...
|-------|-----------|-------------|
| `metadata.name` | VM reconcile path, on create | `"tag-" + pkgutil.SHA1Sum17("<spec.key>:<spec.value>")`. See "Name derivation". |
...
```
name = "tag-" + pkgutil.SHA1Sum17(spec.key + ":" + spec.value)
```
...
`spec.affinity` is immutable (spec NG9), so what changes here is `metadata.labels`. Ownership follows the affinity reference alone, so dropping a label never drops ownership — only the VM's own tag carriage reacts.
