...
func (r *Reconciler) ReconcileNormal(ctx *pkgctx.VirtualMachineSnapshotContext) (ctrl.Result, error) {
	...
	if vmSnapshot.Status.Requested == nil {
		ctx.Logger.V(4).Info("Updating snapshot's status requested capacity")
		vmSnapshot.Status.Requested = []vmopv1.VirtualMachineSnapshotStorageStatusRequested{}
		...
		vmSnapshot.Status.Requested = requested
		ctx.Logger.V(5).Info("Updated vmSnapshot requested capacity", "requested", vmSnapshot.Status.Requested)
		// Enqueue the storage classes to sync corresponding SPU.
		for _, requested := range vmSnapshot.Status.Requested {
			ctx.StorageClassesToSync.Insert(requested.StorageClass)
		}
	}

	if vmSnapshot.Status.Used == nil {
		vmSnapshot.Status.Used = &resource.Quantity{}
	}
	...
	// TODO(lubron): Now the format is always set to decimalSI since the size is int64 and it's not a multiple of 1024.
	// Should consider converting to an approximate number of GB like below did?
	// https://github.com/vmware-tanzu/vm-operator/blob/main/pkg/providers/vsphere/vmlifecycle/update_status.go#L898
	format := resource.DecimalSI
	if size%1024 == 0 {
		format = resource.BinarySI
	}
	total := resource.NewQuantity(size, format)
	vmSnapshot.Status.Used = total
