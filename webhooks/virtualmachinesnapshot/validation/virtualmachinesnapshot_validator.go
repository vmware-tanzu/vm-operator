...
func (v validator) ValidateCreate(ctx *pkgctx.WebhookRequestContext) admission.Response {
	...
	vmRef := vmSnapshot.Spec.VMRef
	if vmRef != nil {
		if vmRef.Kind != "" && vmRef.Kind != virtualMachineKind {
			fieldErrs = append(fieldErrs, field.Invalid(vmRefField.Child("kind"),
				vmRef.Kind, fmt.Sprintf("must be %q", virtualMachineKind)))
		}

		if vmRef.APIVersion != "" {
			gv, err := schema.ParseGroupVersion(vmRef.APIVersion)
			if err != nil {
				fieldErrs = append(fieldErrs, field.Invalid(vmRefField.Child("apiVersion"),
					vmRef.APIVersion, "must be valid group version"))
			} else if gv.Group != vmopv1.GroupName {
				fieldErrs = append(fieldErrs, field.Invalid(vmRefField.Child("apiVersion"),
					vmRef.APIVersion, fmt.Sprintf("group must be %q", vmopv1.GroupName)))
			}
		}
		...
		if vmRef.Name == "" {
			...
			if err := v.validateVMNotVKSNode(ctx, vmRef.Name, vmSnapshot.Namespace); err != nil {
...
func (v validator) validateVMNotVKSNode(ctx *pkgctx.WebhookRequestContext, vmName string, namespace string) error {
