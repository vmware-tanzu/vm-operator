...
- [ ] T061 [S5.a] Register ConfigTarget controller + webhook in `controllers/controllers.go` and `webhooks/` suite files
...
> **Implementation note (T063–T065):** metadata.name (not spec.id) is used as the cluster MoID key, per the controller-workflows.md naming convention. `GetVirtualMachineConfigTarget` was placed directly on `vSphereVMProvider` in `vmprovider.go` — consistent with other single-purpose vSphere queries in that file (`DoesProfileSupportEncryption`, `GetStoragePolicyStatus`) — rather than the `pkg/providers/vsphere/environment_browser.go` sketched in the plan; that file can still be introduced later once `QueryConfigOptionEx` (S6) and the per-host iteration (S5.c) grow the surface area. T060–T062 (webhook) and T061's registration in `controllers/controllers.go` are **not yet done** — they remain open and are tracked as follow-up work (this PR's controller is not wired into `controllers/controllers.go`).
...
- [ ] T071 [S5.e] E2E test — `test/e2e/vmservice/configpolicy/configtarget_test.go`: `ConfigTarget.status` populated from real EB; per-host SR-IOV entries with `hostMoID` on SR-IOV-capable clusters; `ConfigTarget.status.maxHardwareVersion` equals the max of `config.defaultHardwareVersion` across the cluster's hosts.
