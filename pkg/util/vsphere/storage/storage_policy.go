...
func GetStoragePolicyStatus(
	...
	profileID string) (polv1.StoragePolicyStatus, error) {
	...
	var status polv1.StoragePolicyStatus
	...
	p, c, err := getCapabilityProfileAndConstraints(
		...
		profileID)
	if err != nil {
		return polv1.StoragePolicyStatus{}, err
	}
...
func getStorageClassNames(
	...
	profileID string,
	status *polv1.StoragePolicyStatus) error {
...
func getAssociatedDatastores(
	...
	status.Datastores = make([]polv1.Datastore, len(dsRefs))
	...
	for i, ds := range datastores {
		...
		case vimtypes.HostFileSystemVolumeFileSystemTypeVsan,
			...
			status.Datastores[i].Type = polv1.DatastoreTypeVSAN

		default: // VMFS
			status.Datastores[i].Type = polv1.DatastoreTypeVMFS
		}
	}
	return nil
}
					...
					for _, ci := range cap.Constraint {
						for _, pi := range ci.PropertyInstance {
							if pi.Id == "SectorSize" {
								switch pi.Value {
								case "512n", "512N":
									status.DiskFormat = polv1.DiskFormat512n
								case "4kn", "4Kn", "4KN":
									status.DiskFormat = polv1.DiskFormat4k
								default:
									return fmt.Errorf(
						...
						for _, pi := range ci.PropertyInstance {
							if pi.Id == "VolumeAllocationType" {
								...
								case "Conserve space when possible":
									status.DiskProvisioningMode = polv1.DiskProvisioningModeThin
								case "Reserve space":
									status.DiskProvisioningMode = polv1.DiskProvisioningModeThick
								case "Fully initialized":
									status.DiskProvisioningMode = polv1.DiskProvisioningModeThickEagerZero
								default:
									return fmt.Errorf(
