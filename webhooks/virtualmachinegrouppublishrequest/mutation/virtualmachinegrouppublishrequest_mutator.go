...
// +kubebuilder:webhook:path=/default-mutate-vmoperator-vmware-com-v1alpha4-virtualmachinegrouppublishrequest,mutating=true,failurePolicy=fail,groups=vmoperator.vmware.com,resources=virtualmachinegrouppublishrequests,verbs=create,versions=v1alpha4,name=default.mutating.virtualmachinegrouppublishrequest.v1alpha4.vmoperator.vmware.com,sideEffects=None,admissionReviewVersions=v1;v1beta1
...
func (m mutator) Mutate(ctx *pkgctx.WebhookRequestContext) admission.Response {
	...
	return m.mutateCrate(ctx)
}

func (m mutator) mutateCrate(ctx *pkgctx.WebhookRequestContext) admission.Response {
	...
	if len(groupPubReq.Spec.VirtualMachines) == 0 {
		vms, err := m.retrieveVMGroupMembers(ctx, ctrlclient.ObjectKey{Namespace: groupPubReq.Namespace, Name: groupPubReq.Spec.Source})
		if err != nil {
			if apierrors.IsNotFound(err) {
				return admission.Errored(http.StatusNotFound, err)
			}
		...
		groupPubReq.Spec.VirtualMachines = vms
	}
	// ensure vms are unique and sorted
	groupPubReq.Spec.VirtualMachines = sets.List(sets.New(groupPubReq.Spec.VirtualMachines...))
...
func (m mutator) retrieveVMGroupMembers(ctx *pkgctx.WebhookRequestContext, vmGroupKey ctrlclient.ObjectKey) ([]string, error) {
	var vms []string

	vmGroup := &vmopv1.VirtualMachineGroup{}
	if err := m.client.Get(ctx, vmGroupKey, vmGroup); err != nil {
		return nil, err
	}
	...
	for _, member := range vmGroup.Status.Members {
		switch member.Kind {
		case "VirtualMachine":
			vms = append(vms, member.Name)
		case "VirtualMachineGroup":
			groupMembers, err := m.retrieveVMGroupMembers(ctx, ctrlclient.ObjectKey{Namespace: vmGroupKey.Namespace, Name: member.Name})
			if err != nil {
				return nil, err
			}
			vms = append(vms, groupMembers...)
		default:
