...
func intgTestsMutating() {
	...
	When("spec.target is omitted without default content library", func() {
		BeforeEach(func() {
			...
			expectedCreateErr = NotFoundRESTError("ContentLibrary.imageregistry.vmware.com", "")
		})
		It("should deny without CL not found error", func() {})
	})
	...
	When("spec.virtualMachines is omitted without the VirtualMachineGroup created", func() {
		BeforeEach(func() {
			...
			expectedCreateErr = NotFoundRESTError("VirtualMachineGroup.vmoperator.vmware.com", ctx.vmGroupPubReq.Spec.Source)
		})
...
func NotFoundRESTError(kind, name string) *errors.StatusError {
	return &errors.StatusError{
		ErrStatus: metav1.Status{
			Code:    http.StatusNotFound,
			Status:  metav1.StatusFailure,
			Message: fmt.Sprintf("admission webhook %q denied the request: %s %q not found", WebhookName, kind, name),
		},
	}
}
