...
var ErrInfraMaintenanceFault = errors.New("vm power operation failed due to infrastructure in maintenance")
...
// the provided task info indicates the failure is a NoCompatibleHost fault
// caused by the VM's host being in infrastructure maintenance, and with the
// task's fault message when one is available. Named for its sole caller, the
// hard power op path. Exported so its wrapping behavior can be unit tested
// without driving a real vCenter power-op task failure.
