	...
	for _, di := range info.Disks {
		...
		if volume, ok := info.Volumes[di.Target.String()]; ok {
			...
			case "", corev1.ClaimPending:
				...
				logger.Info("PVC is pending, ensuring CnsRegisterVolume exists",
					"pvc", pvc.Name,
					"isFCD", di.FCD)
			...
			case corev1.ClaimBound:
				...
				logger.V(4).Info("PVC is bound, cleaning up CnsRegisterVolume",
					"pvc", pvc.Name,
					"isFCD", di.FCD)
...
// for a VM where the corresponding PVC is bound or the CnsRegisterVolume has
