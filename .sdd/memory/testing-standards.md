...
|--------|---------|---------|
| `_unit_test.go` | Unit tests (mocked dependencies, fake client) | `virtualmachinesnapshot_controller_unit_test.go` |
| `_intg_test.go` | Integration tests (envtest with real API server) | `virtualmachinesnapshot_controller_intg_test.go` |
| `_suite_test.go` | Ginkgo suite setup and registration | `virtualmachinesnapshot_controller_suite_test.go` |

Please note, it is also fine to forgo the use of `_unit_test.go` and `_intg_test.go` distinctions and just use `_test.go`, using test labels to distinguish between the types of tests.
...
func intgTestsReconcile() {
    ...
    BeforeEach(func() {
        ctx = suite.NewIntegrationTestContextForVCSim(...)
    })
