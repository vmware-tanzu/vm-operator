...
| E2E coverage ships with cluster-observable behavior | Done | Scenarios 1–3 (Mandatory `AutomaticVMEvictionPolicy` tagging + match-widening re-evaluation; explicit `BestEffortRestartPolicy` reference plus combinations; the `InfraInMaintenance` condition) shipped in `test/e2e/vmservice/vmservice/computepolicies/vmevictionpolicy.go`, wired into `vmservice_test.go`. Scenario 4 (CRD-presence gating) is closed as not deliverable as e2e with the current harness (no capability-toggle mechanism exists for any feature) and is already thoroughly unit-tested instead — see "Test strategy" and `tasks.md` T030. |
...
- **E2E** (mandatory per `e2e-sync-with-changes.md` — cluster-observable behavior), status per scenario:
  ...
  3. **Done** (`tasks.md` T029): four single-VM `It`s in `vmevictionpolicy.go` drive this against a real host, gated by this `Spec`'s existing `consts.VMEvictionCapabilityName` check. (a) A VM matching a Mandatory `AutomaticVMEvictionPolicy` is relocated off its host on maintenance-mode entry and stays `Synced`. (b) A VM matching an Optional `BestEffortRestartPolicy` is restarted on another host and stays `Synced`. (c) A VM pinned to its host via a real, mandatory, directly-created DRS VM/Host affinity rule (`pinVMToHostViaDRSRule`) — mimicking a real-world VM DRS cannot evacuate, e.g. one with a PCI-passthrough/GPU device — surfaces `VirtualMachinePowerStateSynced=False` with reason `InfraInMaintenance` while pinned and in maintenance (`spec.powerState=PoweredOn`, `status.powerState=PoweredOff`), then reverts to `Synced` on the same host once maintenance ends. (d) Same pin, but the VM also explicitly references an Optional `BestEffortRestartPolicy`; the restart path fails with a different error shape that the `InfraInMaintenance` fault check doesn't match, so it surfaces `False`/`NotSynced` instead, reverting the same way. All four skip on a single-host cluster. Originally landed as a single 3-VM spec pinning two VMs via a real, directly-created `vm_host_affinity` `ComputePolicy`/`InfraPolicy` (`pinVMToHost`); replaced after that capability proved to be only a soft/preferential DRS constraint a routine DRS pass could override, making it unreliable for pinning a VM in place. Scenario (c)/(d) no longer depend on `consts.IaaSComputePoliciesCapabilityName`, since they no longer go through a WCP `ComputePolicy`/`InfraPolicy` at all. The `VMEviction`-off path isn't separately exercised — the whole spec file is skipped via capability gating when the feature is off, and that baseline is already covered elsewhere.
  4. **Closed, not e2e** (`tasks.md` T030): both CRDs are absent from the cluster when the shared capability/feature flag is off, present when on, including install/cleanup transitions — already thoroughly covered by `pkg/crd/crd_test.go`'s fake-client unit tests. Not added as e2e: no capability check in `test/e2e/vmservice/skipper` (for this or any other capability-gated CRD) can flip a Supervisor capability at runtime, only read it — so there is no way to drive this scenario against a live cluster with the current harness, independent of anything specific to this feature.
...
No open violations. Scenario 4 of the required E2E coverage (`e2e-sync-with-changes.md`) is deliberately not an e2e spec — see "Test strategy" item 4 and `tasks.md` T030 for why it's fully covered by a unit test instead.
