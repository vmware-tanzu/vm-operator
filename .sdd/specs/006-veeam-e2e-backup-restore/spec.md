...
- For the **disk-only restore** scenario, E2E MUST use Veeam's virtual disk restore action (restoring a single virtual disk directly) in place of the current mimicry of relocating a virtual disk and manually stripping its cloud-native storage identity.
...
- E2E MUST clean up the temporary Veeam backup job it created — and any restore points or artifacts that job produced — whether the test passes or fails, so repeated CI runs do not accumulate orphaned Veeam jobs.
...
- ~~Should disk-only restore use file-level restore or a full VM restore with the rest discarded?~~ **Resolved**: use Veeam's dedicated "virtual disk restore" action (see `research.md`), which restores a single virtual disk directly rather than requiring a full VM restore to be discarded down to one disk.
