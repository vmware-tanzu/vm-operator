...
func (r *Reconciler) ReconcileNormal(ctx *pkgctx.VolumeContext) error {
	...
	// Get volumeSpec for constructing the volumeStatus.
	volumeSpecForBatchAttachment, processErr := r.processBatchAttachmentAndReturnVolumeSpec(
		ctx,
		batchAttachment,
		volumeSpecForBatch,
	)
	...
	volumeStatusesForBatchAttachment := r.getVMVolStatusFromBatchAttachment(
		ctx,
		batchAttachment,
		volumeSpecForBatchAttachment,
		existingVMManagedVolStatus,
	)

	volumeStatusesForLegacyAttachments := r.processLegacyAttachmentsAndReturnVolumeStatus(
		ctx,
		legacyAttachments,
		attachmentsToDelete,
		existingVMManagedVolStatus,
	...
	updateVMVolumeStatus(
		ctx,
		volumeStatusesForBatchAttachment,
		volumeStatusesForLegacyAttachments,
	)
...
// processBatchAttachmentAndReturnVolumeSpec processes the batch attachment and returns the
// volume specs constructed for the batch attachment.
func (r *Reconciler) processBatchAttachmentAndReturnVolumeSpec(
	ctx *pkgctx.VolumeContext,
	...
	// If there is no volumeSpecs to be created using BatchAttachment, just
	// delete existing BatchAttachment if there is one.
	if len(vmVolumeSpecsForBatch) == 0 {
		...
		if existingAttachment != nil {
			...
			if err != nil && !apierrors.IsNotFound(err) {
				return volumeSpecs, fmt.Errorf("failed to delete CnsNodeVMBatchAttachment: %w", err)
			}
		}
...
// processLegacyAttachmentsAndReturnVolumeStatus processes the legacy attachments and returns the
// volume statuses that are tracked by the legacy attachments.
func (r *Reconciler) processLegacyAttachmentsAndReturnVolumeStatus(
	ctx *pkgctx.VolumeContext,
	...
	for _, vol := range vmVolumeSpecsForLegacy {
		...
		if att, ok := legacyAttachments[attachmentNameForVol]; ok {
			...
			if vol.PersistentVolumeClaim.ClaimName == att.Spec.VolumeName {
				...
				// volume. Since its status might still be Attached, which will
