...
func VMPublishRequestSpec(ctx context.Context, inputGetter func() VMPublishRequestSpecInput) {
	Context("VirtualMachinePublishRequest", Ordered, func() {
		...
		Context("CLS Content Library", Ordered, func() {
			...
			It("should preserve vAppConfig properties on a VM deployed from the published image", Label("experimental"), func() {
		...
		Context("Requested Capacity Estimation", Ordered, func() {
			...
			It("should compute the requestedCapacity annotation from the VM's actual used storage, not its provisioned disk size", Label("experimental"), func() {
