		...
		When("the object needs to be upgraded", func() {
			...
			Context("Feature version tracking", func() {
				...
				When("TelcoVMServiceAPI feature is enabled", func() {
					...
					When("VC VM has an more EthCards than VM Spec interfaces", func() {
						BeforeEach(func() {
							moVM.Config.Hardware.Device = []vimtypes.BaseVirtualDevice{
						...
						It("should backfill NIC type from moVM and ignore extra", func() {
							assertUpgraded()
							assertFeatureVersion("9")
							Expect(vm.Spec.Network.Interfaces[0].Type).To(Equal(vmopv1.VirtualMachineNetworkInterfaceTypeVMXNet2))
						})
					})
