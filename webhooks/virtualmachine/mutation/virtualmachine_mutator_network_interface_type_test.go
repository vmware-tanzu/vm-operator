...
var _ = Describe(
	"SetDefaultNetworkInterfaceTypesOnUpdate",
	Label(
		testlabels.API,
		testlabels.Mutation,
		testlabels.Webhook,
	...
	func() {
		...
		expectMutationSuccess := func() {
			wasMutated, err := mutation.SetDefaultNetworkInterfaceTypesOnUpdate(vm)
		...
		Context("VM has not been feature upgraded", func() {
			...
			When("VM has interface with empty type", func() {
				...
				It("should not mutate", func() {
					expectNoMutation()
					Expect(vm.Spec.Network.Interfaces[0].Type).To(BeEmpty())
				})
			})
		})

		Context("VM has been feature upgraded", func() {
			BeforeEach(func() {
				vm.Annotations[pkgconst.UpgradedToFeatureVersionAnnotationKey] = vmopv1util.FeatureVersionTelcoVMServiceAPI.String()
			})
