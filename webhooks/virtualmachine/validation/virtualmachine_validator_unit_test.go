		...
		DescribeTable("PVC with StorageClass",
			...
			Entry("WaitForFirstConsumer",
				testParams{
					...
					validate: doValidateWithMsg(
						`spec.volumes[1].persistentVolumeClaim: Invalid value: "WaitForFirstConsumer": PVC with WaitForFirstConsumer StorageClass is not supported for VirtualMachines`),
				},
			),
		)
	})
